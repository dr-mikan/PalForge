...
`Pal{ ... }` 接收一个 `Pal.Spec`，返回一个 `Pal.Handle`：
...
- **每个字段和它的说明。** `---@field` 行里 `#` 后面的文字，就是规格声明中的 `doc =`
  字符串，所以把鼠标停在 `maxStack` 上会显示 *inventory stack ceiling (default 1)* ——
  默认值也包含在内。
- **只允许几个取值的字段，会给出一份很短的菜单。** 带 `values` 列表的字段会有自己的具名类型。
  一共五个：`Mesh.Spec.Kind`、`Item.Spec.Category`、`Building.Spec.Mesh.Kind`、
  `Skill.Spec.Kind` 和 `Audio.Spec.Kind`。
  在 `kind =` 后面打一个引号，出现的正好是
...
文件里有 18 个类，每种已声明的结构一个：`Pal.Spec`、`Pal.Spec.Events`、`Pal.Spec.Material`、
`Item.Spec`、`Item.Spec.Recipe`、`Item.Spec.Events`、`Building.Spec`、`Building.Spec.Mesh`、
`Building.Spec.Material`、`Building.Spec.Events`、`Skill.Spec`、`Skill.Spec.Events`、
`Effect.Spec`、`Effect.Spec.Events`、`Audio.Spec`、`UI.Spec`、`Mesh.Spec` 和 `Coord`。
...
它会把 `Scripts/palforge/types.lua` 写到你给它的根目录下面，默认是你当前所在的文件夹，所以
你也可以从别的地方运行它：
...
改了默认值、重写了说明，都算。`types.lua` 是提交进仓库的，所以重新生成的文件应该和规格改动
放在同一个提交里。
...
| `doc = "..."` | `#` 后面的文字 |
| `check = schema.nonEmpty` | 什么都不生成 —— check 只在定义时运行 |
| 没有 `type` | `any` |
