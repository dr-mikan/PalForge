...
silent through fights that certainly landed damage. That is settled negatively now, and
structurally rather than by exhaustion — `skill-hit-source` read every field of every damage
struct this build declares (40 + 6 + 12) and **not one of them names a waza**, so there is nothing
to correlate a hit with and no amount of further hooking can change it. Two building hooks have no
channel at all, `onLeftClick` and `onBreak`, because nothing in the dumps can feed one. The channel
...
subscribed to, which is the party path. `ctx.via` names which one fired. **"This pal did not
exist a moment ago" is now measured too**: `pf_hook pal-spawned-fresh` timestamped every firing
against `world.ready` on 2026-08-02 and recorded 27 firings, 17 of them nowhere near a world load.
It does still fire during the load storm as well, so keep `onSpawned` idempotent — the source
already dedupes per actor within one second.
</Callout>
