...
written to `state/entities_<saveId>.json` under the mod folder. `saveId` is read off
`PalGameInstance`: the save DIRECTORY name first (`GetSelectedWorldSaveDirectoryName`, or
its backing property), then the world's display name, sanitised and prefixed `w_`. When
neither answers it falls back to the shared bucket `world`.
</Step>
