...
它返回同样的九个 api 成员，外加第十个——`.store`，也就是这个包自己保存的状态，它没法指到别的包的
文件上。八个构造函数被包了一层，使定义在 `object_manager.withPack` 内部执行；域以具名
...
包 id 现在同时也是**文件名**——保存的状态住在 `state/<save>/<packId>.json`——所以有三个 id 会在你写下
它的地方被拒绝：`_save`、`_unowned` 和 `_quarantine`，它们是 PalForge 在那个目录里已经占着的名字。
`opts.version` 可以和 `depends`、`recommends` 一起传，它描述的是你的包，而不是框架。那个文件里装着
什么、删掉它会怎样，都在[保存的状态](/docs/concepts/saved-state)里。
...
- 包 id 同时也是它保存状态所在的文件名，所以 `_save`、`_unowned`、`_quarantine` 会被拒绝。
