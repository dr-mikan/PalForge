...
boss:spawn(Player.coordinate())   -- the action for putting one in the world; see Pal
...
ここから 2 つのことが言えます。登録は `pcall` の中で走るので、レジストリ側の問題が定義の
呼び出しを壊すことはありません。そしてキーは **あなたが書いたとおりの id** で、コロンも含み
ます。`"example:Boss"` は `"example_Boss"` ではなく `"example:Boss"` として格納されます。
...
`om.all` はコピーを返すので、そこに書き込んでも何も変わりません。一度登録した id は登録された
ままです。変えたいときは定義し直します。
...
FName が `packid_name` になります。PalSchema の行はすべての Mod で共有されるので、この接頭辞が
パック同士の衝突を防ぎます。
...
名前空間つき id の前後どちらの区間も、英数字かアンダースコアでなければなりません。この検査が
走るのは id を **解決** するときで、定義するときではありません。定義時に `id` にかかるのは、
空でない文字列かどうかだけです。
