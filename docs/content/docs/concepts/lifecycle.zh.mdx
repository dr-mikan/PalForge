...
候选都注册成功，却在确实打出伤害的战斗中始终沉默。这一条现在是以否定的方式定下来的，而且是结构上
的定论，不是“试到累了” —— `skill-hit-source` 读遍了这个构建声明的每一个伤害结构体的每一个字段
（40 + 6 + 12 个），**没有一个是在说技能**，所以根本没有东西可以把一次命中关联上去，再挂多少钩子
也改不了。建筑物的 `onLeftClick` 和 `onBreak` 则连频道都没有，因为翻遍转储也找不到能喂给它的东西。
哪条是哪种，下面的频道表都写了。
...
是哪一个发火了由 `ctx.via` 说明。**“这只帕鲁刚才还不存在”现在也量到了**：`pf_hook
pal-spawned-fresh` 在 2026-08-02 把每一次发火都相对 `world.ready` 打了时间戳，记录到 27 次发火，
其中 17 次离世界加载八竿子打不着。它在加载风暴里同样还会发火，所以请把 `onSpawned` 写成幂等的
—— SOURCE 侧也已经按 actor 做了一秒内的去重。
</Callout>
