...
後ろ 2 段階はあるのに、まだ何も流し込まれないチャンネルもあります。`item.craft` がそれで、
`onCraft` ハンドラは受理されますが実行されません。その処理は、実際に動く場所に書いてください。
作ったアイテムがインベントリに入った時点で `onObtain` が発火します。
...
`event.start()` が実際に走るのは一度だけで、二度目以降の呼び出しは何もしません。
...
建築物は **ライブインスタンス**、つまりイベントが起きたその 1 棟に解決されます。パルとアイテムは
1 体ずつ・1 個ずつ追跡されていないので、登録済みの **クラス** に解決され、ハンドラは `ctx` から
アクターを読み取ります。定義していないバニラのパルやアイテムは何にも解決されず、フックは
呼ばれません。

パルとアイテムの照合は 2 段階です。まず ID の完全一致をテーブル参照 1 回で試し、外れたらその
ドメインの登録済み ID を走査して、各 ID を解決した DataTable 行名とゲームが報告した ID を
突き合わせます。名前空間付きの ID にもイベントが届くのはこのためです。ゲームがスポーンさせるのは
`BP_example_Boss_C`、報告されるアイテム行は `example_Ration` で、コロン形式が現れることは
ありません。

## チャンネル

チャンネルは、名前の付いた 1 つの瞬間です。`event.CHANNELS` が全リストで、宣言順に並んでいます。
...
| `world.ready` | ゲートが開いた後に最初に完走した再構築スキャン。ゲート自体は ready ウォッチで、`LoopAsync(1000)` が `PalPlayerCharacter` をポーリングし、5 回連続で有効だったときに開きます | ペイロードなし | 全ライブ建築物インスタンスの `onWorldReady`。その最初のスキャンが追跡を済ませています |
...
`onBuild`、`onLeftClick`、`onBreak` はスペックとしては受理されますが、呼ばれることはありません。
`onBuild` の代わりに `onPlace`、`onBreak` の代わりに `onRemove` を使い、プレイヤーの操作には
`onRightClick` を使ってください。
...
対象のアクターを走査してください。このページの最後にレシピがあります。
</Callout>
...
ディスパッチの購読者がありますが、emit するものがないので、`onCraft` と `onDiscard` は動きません。
...
<Callout type="warn">
`item.use` の `ctx.actor` はネイティブ呼び出しの第 1 引数そのもので、ゲーム内で確認されている
シグネチャではこの引数はキャラクターではなくアイテムデータオブジェクトです。対象は第 2 引数の
`FPalInstanceID` ですが、こちらは一度もプローブされていません。使うのは `ctx.itemId` です。
プレイヤーが必要な場合は、`ctx.actor` をポーンとして当てにせず `Player.character()` から取得して
ください。
</Callout>
...
すぎず、ステータスアイコンは変化しません。ゲームプレイはハンドラの中に書いてください。体力を
削る、アイテムを渡す、何かをスポーンさせる、といった処理です。
</Callout>

### Skill

ゲームの側からスキルを発火させるものはありません。`skill.*` を emit するチャンネルもありません。
動作するのは自分で呼び出す方法で、クールダウンは Lua 側で強制されます。
...
`state/entities_<saveId>.json` に書き込まれます。ワールドの GUID を読めなかった場合、`saveId` は
`world` にフォールバックします。
</Step>
...
- 周期処理はすべて 500 ms のハートビート 1 本で動きます。建築物の `onTick`、エフェクトの
  タイミング、`event.every(ms, fn)` がこれに乗っています。
- 受理されるが呼ばれないフックがあります。パルの `onTick`、アイテムの `onCraft` と
  `onDiscard`、建築物の `onBuild`、`onLeftClick`、`onBreak` です。その処理は `tick` か、
  実際に発火するフックから動かしてください。
- スキルはひとりでに発動しません。`:activate` や `:hit` を自分のコードから呼びます。
