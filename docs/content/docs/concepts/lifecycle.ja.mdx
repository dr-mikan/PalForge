...
確実にダメージが入った戦闘のあいだ沈黙し続けました。これは現在、否定側で決着しており、しかも
総当たりではなく構造として決着しています。`skill-hit-source` がこのビルドの宣言するダメージ構造体の
全フィールド（40 + 6 + 12）を読み、**そのどれ一つとして技を名指ししていません**。ヒットを技に
突き合わせる材料がそもそも存在しないので、これ以上フックを増やしても結果は変わりません。
...
ます。**「このパルはさっきまで存在しなかった」も、いまや計測済みです。**2026-08-02 に `pf_hook
pal-spawned-fresh` が発火のたびに `world.ready` からの時刻を記録し、発火 27 回、うち 17 回は
ワールドロードとまったく無関係な時刻でした。ロードの嵐の中でも依然として発火するので、`onSpawned`
は冪等に書いてください（SOURCE 側でもアクター単位・1 秒以内の重複は除去されます）。
</Callout>
...
`state/<saveId>/<mod id>.json` に書き込まれます。セーブごとに 1 ディレクトリ、mod ごとに 1 ファイル
です。`saveId` は `PalGameInstance` から読みます。
