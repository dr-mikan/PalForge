...
that `Pal{ ... }` takes a `Pal.Spec` and hands back a `Pal.Handle`:
...
- **Every field with its description.** The text after `#` in a `---@field` line is the
  `doc =` string from the spec declaration, so hovering `maxStack` shows *inventory stack
  ceiling (default 1)* — the default value included.
- **A short menu where only a few values are allowed.** A field with a `values` list gets its
  own named type. There are five: `Mesh.Spec.Kind`, `Item.Spec.Category`,
  `Building.Spec.Mesh.Kind`, `Skill.Spec.Kind` and `Audio.Spec.Kind`.
...
The file holds 18 classes, one per declared shape: `Pal.Spec`, `Pal.Spec.Events`,
...
Run the generator whenever a spec changes. It is a dev tool, and it runs on a plain Lua
interpreter with the game closed:
...
It writes `Scripts/palforge/types.lua` under whichever root you give it, defaulting to the
folder you are standing in, so you can also run it from elsewhere:
...
allowed value, a changed default, a reworded description. `types.lua` is committed to the
repo, so the regenerated file belongs in the same commit as the spec change.
