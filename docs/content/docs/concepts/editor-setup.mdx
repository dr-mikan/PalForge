...
The file holds **37 classes and 30 aliases**, and it is long: the generator reports 8752
lines. Thirty of the classes are the declared shapes, one per name `schema.all()` returns —
`Mesh.Spec`, `Pal.Spec` with its `Events` and `Material`, `Item.Spec` with its `Recipe` and
`Events`, the four `Building.Spec.*`, `Skill.Spec` and its `Events`, `Effect.Spec` and its
`Events`, `Audio.Spec`, the eleven `UI.Node.*` node shapes, `UI.Spec` and `UI.Spec.Host`,
and `Coord`.
