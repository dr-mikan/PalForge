...
<Callout type="error" title="このバージョンでできない 1 つのこと">
**自分で用意したサウンドは再生できません。** `Audio` の定義に自作の音声ファイルを指定しても、
受け付けられたうえで何も起きません。再生できるのはゲームに最初から入っているサウンドだけで、
その数は数千あります。カタログは [Audio](/docs/api/audio) にあります。
