...
- **Effect** — `onApply`、`onTick`、`onStack`、`onExpire` がすべて走ります。時間の管理は
  PalForge 自身が 0.5 秒間隔の鼓動で行います。ゲーム本体の状態異常アイコン（毒・炎上・氷結）は
  切り替えません。対象に何をするかは、あなたのハンドラーが決めます。
...
- `Audio.Handle:setVolume` は `false` を返すだけで何も変わりません。自分で用意した `soundFile` も
  解決はされますが再生されません。ゲーム内のサウンド id を指定してください。
