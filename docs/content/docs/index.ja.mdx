...
どこまで自動で動くかはドメインによって違います。

- **Building** — 一番充実しています。`onPlace`、`onLoad`、`onRightClick`、`onRemove`、
  `onTick`、`onWorldReady`、`onWorldLeft` はすべて、生きている構造物に対して走ります。
  `onBuild` も建築が完了した瞬間に走りますが、構造物ができる前に届くため、生きた構造物では
  なく定義そのものが渡されます。あとから読み込まれた構造物は `onWorldReady` を取り逃すので、
  構造物ごとの初期化処理は `onLoad` に置いてください。
- **Pal** — 5 つすべて走ります。`onCaptured`、`onDamaged`、`onDeath` は確認済みのゲーム側の
  呼び出しから来ます。`onSpawned` は候補のフックに乗っており、新規出現なのか再初期化なのかは
  まだ確定していないので、2 回走っても平気な書き方にしておいてください。`onTick` はワールド内の
  パルを走査する仕組みが駆動していて、おおよそ 3 秒ごとです。
- **Item** — `onObtain` と `onUse` が走ります。`ctx.itemId`、`ctx.count`、`ctx.actor` が
  付いてきます。
- **Effect** — `onApply`、`onTick`、`onStack`、`onExpire` がすべて走ります。時間の管理は
  PalForge 自身が 0.5 秒間隔の鼓動で行います。定義に `nativeStatus` を書けば、エフェクトが動いて
  いるあいだゲーム本体の状態異常が点灯します。それ以外に対象へ何が起きるかは、あなたの
  ハンドラーが決めます。
- **Audio / Mesh / UI** — ライフサイクルのイベントはありません。サウンドは自分で鳴らし、メッシュは
  身に着けさせ、UI 要素はマウント・更新・アンマウントを自分で行います。`Audio.Handle:stop(actor)` は
  サウンド単位ではなく、そのアクターで鳴っているものをすべて止めます。

<Callout type="error" title="このバージョンでできない 2 つのこと">
パックを設計する前に、この 2 つを前提にしてください。どちらも書くこと自体はできます。途中で
失敗するのではなく、`false` を返して何も変えません。

- **パルをワールドに置けません。** `Pal.Handle:spawn` はどこにもクリーチャーを出しません。動くのは、
  ゲーム自身が出したクリーチャーへの反応です。捕獲・ダメージ・死亡と定期巡回はすべて走ります。
- **サウンドごとの音量はありません。** `Audio.Handle:setVolume` は何も変えず、ゲーム側にも
  サウンド単位の音量が存在しません。カタログから静かなサウンドを選んでください。

このページの残りは今日から動きます。
</Callout>

<Callout type="warn">
次のものは宣言できるので書いても壊れませんが、いまは何も動かしません。実際に走る場所に処理を
置いてください。

- Building の `onLeftClick` と `onBreak` — `onRightClick` と `onRemove` を使ってください。
- Item の `onCraft` と `onDiscard` — `onObtain` か `onUse` を使ってください。
- Skill のハンドラー — ゲーム側がスキルを発火することはありません。`:activate(owner)`、
  `:hit(target)`、`:equip(owner)`、`:unequip(owner)` を自分のコードから呼んでください。パルの
  ハンドラー、建築物の `onRightClick`、キーバインドなどが呼び出し元になります。クールダウンは
  Lua が数えていて、冷却中の `:activate` は `false` を返します。
- 自分で用意した `soundFile` は解決はされますが再生されません。ゲーム内のサウンド id を
  指定してください。
- UI 要素を代わりに更新してくれるものはありません。状態が変わったら自分で `:refresh()` を呼ぶか、
  `:autoRefresh(ms)` で定期的に更新してください。
</Callout>
...
- `Item.Handle:give` はインベントリが実測でどう動いたかを返すので、`false` は「何も届かなかった」
  という意味です。`:spawn` の行はこのバージョンでは何も配置しません。それ以外はすべて動きます。
...
- フィールド名を間違えると呼び出しは止まり、`PalForge: ` で始まるメッセージが正しい候補を
  教えてくれます。
- スキルは自動では発火しません。Item の `onCraft` / `onDiscard` と Building の `onLeftClick` /
  `onBreak` も走りません。実際に走るハンドラーに処理を置いてください。
