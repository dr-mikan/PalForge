...
<Callout type="info">
`:spawn` 返回 `true` 的意思是**已被接受**，不是已经到位。actor 要过几帧才实体化，重新定位跑在
一条重试链上，所以 pal 套件故意不检查它离请求坐标有多远 —— 只检查生成被接受了，以及世界可以被
枚举。
</Callout>
...
- **生成出来的帕鲁会留下。** `pal` 在它的三个实时测试里向 `UPalCheatManager` 要一只 `ChickenPal`。
  整棵树里没有东西能把它们收回去。
- **发出去的道具可能会留下。** `item` 发三个 `Wood`，紧接着调用 `:take(3)`。这个版本上没有确认
  可用的移除调用，所以 `:take` 把一个负的增量推过同一条添加路径，再重新读一次数量；这个测试
  只主张它返回了一个自己确实测出来的布尔值。`false` 表示那三个 Wood 还在你的背包里。
...
- **效果会自己收拾干净。** 唯一的实时 effect 测试在 `pcall` 下把效果加到 pawn 上再移除掉，而
  PalForge 的效果是纯 Lua 的记账 —— 无论哪种情况都不会切换原生的状态异常。

<Callout type="warn">
碰世界的套件请在一个可以丢掉的存档里跑，别在你在意的那个存档里跑。三只鸡和三个木头是很小的一点
乱，但它确实是乱，而框架并没有承诺会去收拾。
</Callout>

一次全绿的运行，并不代表 api 里每个调用都把它名字暗示的活干完了。今天有两个只做了一半，套件
特意把这一点钉住：

- `Audio.Handle:setVolume` 对每个值都返回 `false`，边界值也一样。它目前什么也不做。请改用游戏
  自己的音频选项来调音量，并把那个 `false` 当作诚实的回答。
...
- 碰世界的套件请在可以丢掉的存档里跑：生成出来的帕鲁和发出去的 Wood 会留下。
