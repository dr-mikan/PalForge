...
在标题画面会跳过。另有 10 项正相反：它们验证的是*拒绝*路径 —— 关着的 ready 闸门、没有归属者可
...
那 28 项受世界限制的检查会改为运行，`could not be answered by this session` 的两项也会消失 ——
它们只是因为纯 Lua 进程既没有 `LoopAsync` 也没有 UE4SS 的 `Key` 表才出现的。
...
一共有 **481 项检查**，分成 **14 个**套件。一个套件就是一组检查，覆盖 api 的一个领域。每个套件
...
方向。`28 need a world` 是一句你能照着做的话，`31 skipped` 不是。
</Callout>
...
不带参数的 `test.run()` 只跑那十四个套件，不会更多。PalForge 在启动时还会跑一小组纯 Lua 的检查
...
十九个声明过的钩子，按 `pf_hooks_all` 的运行顺序排列 —— 只读的在前，这样即使中途出事，凡是能打印
的东西都已经在写入发生之前打印完了：
...
十九个里有五个声明了 `writes = true`。`audio-setvolume-audible` 刻意不声明：它确实出声，但它改的
...
可以去跑的事情上。`pf_hooks` 会把十九个全列出来，并附上它们此刻会跳过的理由。
</Callout>
...
文件加载失败不会让这次运行停下：它会记在 `err` 级别，并从 `test.suites()` 里被排除，其余十三个
照跑。
...
- 需要 Palworld 在跑才能做的测量，是 `test/hooks/` 下声明过的钩子：只在 `env.debug` 下加载，用
  `pf_hooks` / `pf_hook <id>` / `pf_hooks_all` 按名字运行，其中会写入存档的三个还需要
