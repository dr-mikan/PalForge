...
**没有哪一次按键能跑完所有检查，而汇总会用文字把这件事说出来。** 31 项检查需要已加载的存档，
在标题画面会跳过。另有十八项正相反：它们验证的是*拒绝*路径 —— 关着的 ready 闸门、没有归属者可
挂载的控件、没有 pawn 时的 `Player.coordinate()` —— 所以只能在没有任何东西可以成功的地方运行。
这十八项里有五项会在世界起来之后跳过，只在标题画面跑；剩下的十三项根本不需要引擎，只会在无头的
`lua5.4` 进程里跑。没有哪一种会话状态下的一次按键能测完全部。请在标题画面按一次，进存档再按一次；
一次全绿的运行，其实是两次运行。

同样这 612 项检查完全不用游戏也能跑，CI 跑的就是这个：
...
**两条路之间差着一项检查，而这不是对不上。** `npm test` 直接调用 `run()`；F1 是经由 `install()`
过去的，而 `install()` 同时也绑定了 F1 —— 于是那项验证*PalForge 已经握着的键会被拒绝、并且拒绝时
点名说出是谁握着*的 `ui` 检查，在 F1 这条路上有一个真的被绑住的键可以拿来拒绝，在 `npm test` 这条
路上则无从看起，只能不带方向地跳过。另外 `install()` 会先跑无头的启动测试包，它会在那 612 项之前
先报出自己那行 `8 passed, 0 failed, 0 skipped`。

上面两组数字都是 2026-08-03 在一个没有任何引擎的纯 `lua5.4` 进程里的实测结果。在游戏里，那 31 项
受世界限制的检查会改为运行，`could not be answered by this session` 的三项里也会消失两项 —— 那两项
只是因为纯 Lua 进程既没有 `LoopAsync` 也没有 UE4SS 的 `Key` 表才出现的。
...
一共有 **612 项检查**，分成 **19 个**套件。一个套件就是一组检查，覆盖 api 的一个领域。每个套件
是 `Scripts/palforge/test/cases/` 下的一个文件。它们针对你这次会话里真实的游戏运行。

| 套件 | 检查数 | 只在存档里跑 | 只在没有引擎时跑 |
| --- | --- | --- | --- |
| `schema` | 28 | 0 | 0 |
| `registry` | 42 | 0 | 0 |
| `definitions` | 23 | 0 | 0 |
...
中间那一列就是受世界限制的 31 项，是从一次真实运行里数出来的。右边那一列是游戏加载之后就跑不了的
十八项，而它其实装着两种不同的闸门：五项调用 `support.needNoWorld`，在标题画面确实会跑
（`store_api`、`building`、`audio`、`player`、`events`）；另外十三项调用 `support.needNoEngine`，
只在无头进程里跑（`store_runtime` 的五项、`ui` 的七项，以及 `mesh` 里的一项）—— 它们验证的是没有
`LoadAsset`、没有归属者、也没有 `FindAllOf` 可以替换时会发生什么，而一个 UE4SS 会话这三样都有。
...
所以由一个声明过的钩子来测量，而不是 F1；它那行跳过会点出负责测量的钩子名字。那个钩子已经跑过了
—— `pal-skills-equip`，在真实存档上 8 通过 / 0 失败 —— 所以这条跳过的意思是"在别处测过了"，
不是"不知道"。
...
方向。`31 need a world` 是一句你能照着做的话，`35 skipped` 不是。
</Callout>
...
`tools/deploy.sh` 写出的那个覆盖文件会把九个写入同意项**逐行注释掉**列出来，这样每一项都是一个
单独的决定。当一整次会话就是要把它们扫一遍时，有一个开关能把它们一次全部打开，并且会明说：

```bash
tools/deploy.sh --writes    # all nine env.debugHooks opt-ins ON. Throwaway save only
```

它之所以是一个单独的开关而不是默认行为，是因为**这九个里有三个撤不回来**：生成出来的帕鲁没有
按个体删除的办法；解锁了的科技没有上锁的办法 —— `UPalCheatManager` 声明了四个解锁入口，而整份头
文件转储里找不到任何反向操作；而教会一个主动技，是这棵树里唯一一次与游戏关闭产生过关联的写入。
（那次关联现在已经定性了：出事的是目标，不是写入 —— 那是个村民。`pal-skills-equip` 要求的是
`PalMonsterCharacter`，它返回了 8 通过 / 0 失败，游戏也一直开着。）

二十五个声明过的钩子，按 `pf_hooks_all` 的运行顺序排列 —— 只读的在前，这样即使中途出事，凡是能打印
的东西都已经在写入发生之前打印完了：

| 钩子 | 需要 | 它把什么问题定下来 |
| --- | --- | --- |
| `game-build-live` | world | `GetBuildVersion` / `GetEngineVersion` / `GetGameName` 的原始字符串，好让声明的版本和运行中的版本真正对上 —— **2026-08-02 跑过三次**：这三个携带的是 *Unreal* 的身份而不是 Palworld 的，版本号来自 `PalGameInstance.DisplayVersion` 和 `PalUtility.GetDisplayVersion`，两者都回答 `v1.0.2.101103` |
...
| `mesh-actor-identity` | world | 以 UE4SS actor 句柄为键的表，在之后的扫描里还能不能命中 —— 整套按 actor 重新取键的做法都压在这次测量上。**2026-08-02 在三次独立会话里都得到确认**：句柄会落空，全名会命中，`rawequal` 为 false 而 `uo.same` 为 true，所以键只能是名字 |
| `item-datatable-row-read` | world | `dt:FindRow(id)` 返回的结构体上能不能直接索引标量或 FName 列 —— **2026-08-02 跑过：能**，`Item.Handle:recipeOf` 会去读游戏那一行正是因为这个结果 |
| `audio-custom-file-loader` | world | 正式版里到底有没有任何办法，把磁盘上的 `.wav` 变成能播放的东西 —— **2026-08-02 跑过**：Wwise 的外部媒体那一半在这个构建上不存在，UE 的播放那一半是活的，剩下的问题是能不能构造一个 `USoundWave` 并把它填上 |
| `building-record-orphans` | world | 针对真实存档的孤儿隔离往返：读进来多少条记录、多少条没有已注册的归属、多少条被隔离 —— 以及最要紧的那条断言：仅仅是本次会话没有加载的包，**没有**被销毁 |
| `store-base-load-cost` | world | 一个真实基地让 store 加载起来要付多少：文件数、字节数、按那些字节计时的解码，以及同样这些记录若合成一个文件会是多少 —— **2026-08-02 跑过：0 字节、0.00 ms、0 个文件**，因为当时一个建筑定义都没注册 |
| `building-actor-streaming` | world, player | `FindAllOf("PalBuildObject")` 会不会不再返回玩家已经走远的基地，以及在多远处开始 —— **仍然没有结果**：操作者走到了 287 m，90 次采样里 7 个结构一个不少地全都还在，所以 Palworld 的流送半径根本就比这更大 |
| `ui-update-event` | world | 给 21 个点名的 UI 函数布上钩子，报告哪些真的触发、按什么顺序 —— **2026-08-02 跑过：其中三个会。** 操作者开关两个界面的过程中，`CommonActivatableWidget:ActivateWidget`、`PalHUDService:Push` 和 `PalHUDService:Close` 都触发了，这就是 `UI.Handle:autoRefresh` 如今搭乘的那个重建信号。另外 18 个一直没有声音，而那是没有输入，不是结论 |
| `pal-spawned-fresh` | world | 把每次 `pal.spawned` 的触发相对 `world.ready` 打上时间戳，好把新生成和加载风暴区分开 —— **2026-08-02 跑过：27 次触发，其中 17 次离世界加载八竿子打不着**，所以这个事件确实是"来了一只新帕鲁"的意思 |
| `skill-hit-source` | world | 确认伤害路径上没有任何东西携带技能 id，并量化为什么把发动和伤害相关联是错的 —— **2026-08-02 跑过：这个构建声明的三个伤害结构体里，没有一个字段是在说技能**（40 + 6 + 12 个字段），于是 `onHit` 是被否定地定下来的，而不是悬着 |
...
| `mesh-texture-import-live` | world, player | 调用 `ImportFileAsTexture2D`，然后对同一个路径再调一次，说清楚缓存是不是替代了重新导入 —— **2026-08-02 跑过**：返回了一个真正的 `Texture2D`，第二次由缓存回答 |
| `mesh-color-change` | world, player, **写入** | 在玩家面前放一个带色的网格，并把颜色改两次 —— **2026-08-02 跑过，而且操作者亲眼看着**：空中的一个箱子依次变成红 → 绿 → 蓝 → 消失 |
| `audio-setvolume-audible` | world, player | 用四档输出总线音量播放同一个声音，好让人判断 `setVolume` 有没有听得见的效果 —— **唯一还开着的那一项。** 2026-08-02 跑过：每一档音量下调用都返回 true，而听的人两次都带着保留地报告说 0.00 **不是**静音 |
| `building-unlock` | world, player, **写入** | 把关于 `Building.Handle:unlock` 能确认的事实全部记下来，剩下确认不了的那一半交给操作者 |
| `item-satiety-write` | world, player, **写入** | 运行中的版本给 `SetFullStomach` 声明的是什么参数表，以及饱食度 / HP 的写入从 Lua 到底够不够得着 —— **2026-08-02 跑过：够得着。** `SetFullStomach` 只吃一个参数，饱食度从 31.648 走到 21.648 又被放了回去，`AddHPByRate` 也落到了。`Item.Spec.restores` 正是建在这次测量上的 |
| `skill-projectile-spawn` | world, pal, **写入** | 打印每一条弹道 / 生成路径声明的参数表，只发射那条不吃参数的，并把吃结构体的那些点名拒绝 —— **2026-08-02 跑过：走完七份参数表，拒绝了六条路径，每一条都吃结构体。** 结论是否定的，并且作为一条把这六条全都带上的拒绝发布出去 |
| `store-save-roundtrip` | world, **写入** | 通过公开接口把一个包的状态写进真实存档的 store，再从真实路径读回来，并在重载之后再跑一次，看它能不能跨过一次世界加载。**2026-08-02 第 1 趟：7 通过**，370 字节写进去又逐字段读了回来 —— 在那之前它还抓出了一个 Linux 套件抓不到的真实缺陷（`ensureDir` 拿 `io.open` 去问一个目录；Windows 说不行）。第 2 趟，也就是跨越一次世界加载的那一半，还欠着 |
| `store-crash-recovery` | world, **写入** | 在真实存档的 store 里种下一个写到一半的文件和一个读不了的文件，报告加载器是从哪一份副本恢复的 —— **2026-08-02 跑过：恢复表的四行在 NTFS 上全部成立**，不只是写在纸上 |
| `save-survives-pack-removal` | world, player, **写入** | 把 PalForge 让这个存档记下的每一个 id 都点名，然后在卸载内容包并重新加载之后，说清楚它们各自变成了什么。**开过一次，从没跑完**；整轮 store 的工作就是为这次测量才开始的 |
| `pal-skills-equip` | world, pal, **写入** | 主动技的写入会不会落到一只真帕鲁身上，以及游戏能不能撑住 —— **2026-08-02 跑过：8 通过 / 0 失败，游戏也一直开着。** `AddEquipWaza` 装上了 `Human_Punch`，并且在一只活着的 `PalMonsterCharacter` 上读了回来，`:forget` 把它取下，`teachAll(pal)` 回答 `2, 2`，`ClearEquipWaza` 清空了配置，之后每个招式都恢复了 |

二十五个里有九个声明了 `writes = true`。`audio-setvolume-audible` 刻意不声明：它确实出声，但它改的
...
可以去跑的事情上。`pf_hooks` 会把二十五个全列出来，并附上它们此刻会跳过的理由。
</Callout>
...
- 按两次：标题画面一次，存档里一次。31 项只在存档里跑，五项只在没有世界时跑，所以没有哪一次按键
  能测完全部 —— 汇总会告诉你是哪一边。
- `npm test` 完全不用游戏也能跑同样这 612 项检查：**576 通过、0 失败、36 跳过**。走 `install()`
  ——也就是 F1 那条路 —— 则是 **577 / 0 / 35**，外加启动测试包的 `8/0/0`。
...
- 需要 Palworld 在跑才能做的测量，是 `test/hooks/` 下声明过的钩子：只在 `env.debug` 下加载，用
  `pf_hooks` / `pf_hook <id>` / `pf_hooks_all` 按名字运行，其中会写入存档的九个还需要
  `env.debugHooks[id] = true` —— `tools/deploy.sh --writes` 能在一个可以丢掉的存档上把这九个
  一次全部打开。
