...
Thirty-seven actions, in four groups: eighteen of them always, plus one per declared hook once
`env.debug` is on.
...
| `pf_hook_<id>` | one generated name per declared hook, nineteen of them |
...
Nineteen measurements cannot be taken with the game switched off. Each is a declared hook under
...
- Nothing exists without `env.dev`, and the nineteen game-required hooks also need `env.debug`.
