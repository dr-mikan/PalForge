...
- 召唤帕鲁要经过 `UPalCheatManager`，也就是游戏自己的管理对象。PalForge 会去找一个，找不到
  就在玩家控制器上自己造一个，所以你只要在世界里就够了。还没有控制器时，生成会记一条警告并
  返回 `false`，包里其他部分照常运行。
- 发放道具不需要这些。`Item.Handle:give` 走的是背包自己的 `AddItem_ServerInternal`。
...
<Callout type="warn">
这里有两样东西还不能用，请绕开它们。`Handle:setVolume` 返回 `false`，什么也不改——请直接挑一个
本身响度就合适的音效事件。`soundFile`（你自己的音频文件）能被接受，然后什么也不做，所以请改用
...
<Callout type="warn">
PalForge 的效果不会点亮游戏本身的状态图标。`EPalStatusEffectType` 是游戏内部的一份列表，
目前没有确认过任何能应用它的调用，所以 `nativeStatus` 只是一个你能读回来的标签，它本身不
做事。效果实际要做的事情，请写进 `onTick`。
</Callout>
...
注册类的数量和实例键取决于你这次会话，行的顺序则不会变。`onActivate` 先跑音效、效果和生成，
再打印自己那一行，所以增益和生成先被报出来——而 `give Ruby x3` 只会在 `:activate` 返回之后
出现。
...
| Skill | 没有 | `:activate()`, `:hit()`, `:equip()`, `:unequip()`, `:cooldownLeft()` |
| Audio | 没有 | `:play()`, `:stop()` |
...
**帕鲁没有出现。** 按坐标生成需要会话里有玩家控制器。没有的话它会记下
`spawn.palAt: no PalCheatManager and none could be constructed (no player controller yet?)`
并返回 `false`；不带 `at` 的 `:spawn()` 报的是同一件事，前缀是 `spawn.pal:`。返回 `true`
表示生成被接受、移动到你给的坐标已经排上了——不表示已经有一只帕鲁站在那里。剩下的都会写进
日志。
