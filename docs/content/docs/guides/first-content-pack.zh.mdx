...
音效、增益、发放、冷却、计数和存档状态，都会在游戏里真的发生。这个版本做不到的只有守护帕鲁
这一步，到了那个位置会再说一次。
...
- 发放道具走的是同一个管理对象，并且返回它实测到的结果：`Item.Handle:give` 在前后各读一次
  背包，只有数量真的上升了才返回 `true`。
- 这个包里有一步在这个版本做不到，而且它是软失败的。`Pal.Handle:spawn` 不会在世界里放出生物：
  它返回 `false`，往日志里写一行，处理函数剩下的部分照常往下走。它仍然写在包里，因为形状是对
  的，也因为“每个调用都检查结果”的包才是不会坏的包。见 [Pal](/docs/api/pal)。
...
<Callout type="warn">
这里有两样东西不能用，请绕开它们。`Handle:setVolume` 返回 `false`，什么也不改，而游戏也没有提供
按声音调节的音量——请直接挑一个本身响度就合适的音效事件。`soundFile`（你自己的音频文件）能被接受，然后什么也不做，所以请改用
目录里游戏自带的 `AkAudioEvent`。`Handle:stop` 也不是按单个声音停的：它会停掉那个 actor 上
正在播放的一切。
</Callout>
...
<Callout type="warn">
`onObtain` 跟的是游戏自己那条“你获得了一件道具”的日志
（`PalPlayerState:AddItemGetLog_ToClient`），拾取、战利品和奖励都会触发它。不进这条日志的
背包添加不在覆盖范围内，所以别指望自己调用的 `:give` 会以 `onObtain` 的形式回到你这里——要
记录就在调用的地方记。`onCraft` 和 `onDiscard` 可以声明，但目前没有任何东西会发送它们。
</Callout>
...
注册类的数量和实例键取决于你这次会话，行的顺序则不会变。`onActivate` 先跑音效、效果和生成尝试，
再打印自己那一行，所以增益和生成先被报出来——而 `give` 那一行只会在 `:activate` 返回之后出现。

其中一行就是这个版本做不到的那一步，它是被如实报告出来的：没有生物被放下。其余每一行都是包在
正常工作，而 `give` 那一行写出了背包前后的数量，因为它的 `true` 就是从那里实测出来的。正因为
处理函数在拿到 `false` 之后还会往下走，你才能这样读日志：哪一步没成，一眼就看得见。
...
| Pal | `onSpawned`, `onDamaged`, `onDeath`, `onCaptured`, `onTick` | `:spawn()`（这个版本什么都放不下）、`:renderOn(actor)`、`:skillsOf()`、`:teachAll(actor)` |
| Item | `onObtain`, `onUse` | `:count()`、`:give(n)`、`:take(n)`（拿走的东西会掉在玩家脚边）、`:iconOf()`、`:recipeOf()` |
...
有四个处理函数可以声明，但目前不会运行，因为游戏里没有东西宣告它们：
`Building.onLeftClick`、`Building.onBreak`、`Item.onCraft` 和 `Item.onDiscard`。建筑物请用
`onRightClick` 和 `onRemove`，道具请用 `onObtain` 和 `onUse`。它们保留可声明，是为了将来找到
来源时，你的包不用改。
...
`Pal.onSpawned` 接在 `BroadcastOnCompleteInitializeParameter` 上，这是一个候选钩子而非确认的
钩子，而且 PalForge 只在世界加载完之后才装上它。请把它当作尽力而为，并让处理函数跑两次也
不出事。
...
这个调用要求会话里已经有 `PalCheatManager`，所以它需要 CheatManagerEnabler 模组。
...
**帕鲁没有出现。** 这个版本里 `:spawn` 不会有帕鲁出现，所以这是预料之中的结果，不是你哪里配错了
—— 细节见 [Pal](/docs/api/pal)。另外还有更早的一种失败值得排除：按坐标生成需要会话里有玩家控制器。
没有的话它连游戏都够不到，并记下
...
根本没有管理对象（打开 `CheatManagerEnablerMod`）、游戏不认识这个道具 ID，或者背包已经放不下。
...
**两个定义用了同一个 id。** 同一种内容里同一个 id 注册两次，后面的会替换前面的，所以最后跑的
那个定义赢。定义放在加载时做，处理函数里用 `X.get(id)` 取，而不是每次事件都重新定义一遍。
...
- `id` 是任何地方唯一必填的字段。带冒号的 id（`"beacon:Flare"`）是你自己的；不带冒号的
  id（`"Altar"`、`"Ruby"`、`"Kitsunebi"`）是游戏里已经有的内容。
