...
<Callout type="warn">
这里有两样东西和名字给人的印象不一样，请绕开它们。`soundFile`（你自己的音频文件）能被接受，然后
什么也不做，所以请改用目录里游戏自带的 `AkAudioEvent`。另外 `Handle:stop` 和 `Handle:setVolume`
都不是按单个声音来的：两者作用的都是那个 actor 上正在播放的一切，所以想让某一个声音更轻，就挑一个
更轻的事件。
</Callout>
...
| Item | `onObtain`, `onUse` | `:count()`、`:give(n)`、`:take(n)`（消耗掉；需要玩家装备着东西）、`:iconOf()`、`:recipeOf()` |
