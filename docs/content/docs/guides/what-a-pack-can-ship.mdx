...
| Texture | Unproven — wired, never once called | Yes |
...
So PalForge today is a framework for **reusing the game's own assets**, with one working
disk route (OBJ) and one wired-but-unobserved one (PNG textures). That is worth knowing before
you model anything.
...
**No run has ever called it.** Nothing has watched a texture come back. Treat a PNG of your own as
unproven rather than broken, and expect to have to find out yourself.
...
-- unproven: a PNG of your own, relative to this file
...
(`BaseColor`, `Base Texture`, `Normal Map`, and the rest), and the writes are declared exactly as
called — but no run has yet watched a colour change on screen. `mesh-color-change` is the declared
hook that measures it.
</Callout>
...
- A PNG texture is wired with the right signature and has never been called; treat it as unproven.
