...
<Callout type="warn">
このドメインには、名前から想像するものとは違うものが 2 つあります。`soundFile`（独自の音声
ファイル）は受け付けられますが何もしないので、カタログにある `AkAudioEvent` を使ってください。
そして `Handle:stop` も `Handle:setVolume` もサウンド単位ではありません。どちらもそのアクターで
鳴っているものすべてに効くので、特定の 1 つを静かにしたいときは静かなイベントを選んでください。
</Callout>
...
| Item | `onObtain`, `onUse` | `:count()`、`:give(n)`、`:take(n)`（消費します。プレイヤーが何かを装備している必要があります）、`:iconOf()`、`:recipeOf()` |
