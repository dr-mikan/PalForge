...
三十七个动作，分四组：始终存在的十八个，外加 `env.debug` 打开之后、每个声明式钩子各生成一个。
...
| `pf_hook_<id>` | 每个声明式钩子生成一个名字，共十九个 |
...
有十九项测量在游戏关着的时候取不到。每一项都是 `test/hooks/` 下的声明式钩子，自己写明屏幕上需要
...
- 没有 `env.dev` 就什么都不存在；十九个需要游戏的钩子还额外需要 `env.debug`。
