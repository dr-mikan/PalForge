...
- `env.dev` 为 false 时这些都不存在。整个 `palforge.test` 模块只在 dev 闸门之下才被 require，而
  `tools/deploy.sh --release` 更是把 `palforge/test/` 从部署出去的树里整个删掉 —— 所以发布版的副本
  既没有 `pf_*` 命令，也没有 F1 和探针按键，内核那唯一一次 `require` 答的是 `absent`。
...
四十二个动作，分四组：始终存在的十八个，外加 `env.debug` 打开之后、每个声明式钩子各生成一个，共二十四个。
...
| `pf_hook_<id>` | 每个声明式钩子生成一个名字，共二十四个 |
...
有二十四项测量在游戏关着的时候取不到。每一项都是 `test/hooks/` 下的声明式钩子，自己写明屏幕上需要
...
这个文件里装的是**名字，不是代码**。`core/autorun.lua` 不写任何 `test/` 底下的名字：这张表是
`test/init.lua` 的 `install()` 通过 `autorun.setActions(M.ACTIONS)` **交**给它的。每一行只是在那
张表里查一次，查不到的名字会被报告并跳过：

```lua
-- core/autorun.lua 拿着别人交给它的表所做的事
local run = M._actions[name]        -- 由 test/init.lua 的 install() 设置一次
if not run then log.warn(name .. ": no action of that name") end
```

在没有测试树的发布版里根本不存在这张表，于是这个队列就是一个零成本的空操作，而不是每次载入世界都
去失败一次 require。所以一个乱入的文件不可能执行按键做不到的事，而会写入的钩子在此之上还有自己的
第二道闸门。

## 小结

- 每一项测量都是 `palforge.test` 的 `ACTIONS` 里的一项；启动日志那一行才是权威清单。
- 没有 `env.dev` 就什么都不存在；二十四个需要游戏的钩子还额外需要 `env.debug`。
