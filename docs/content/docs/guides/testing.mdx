...
That shape is worth copying. A spawn is asynchronous, so a test cannot assert that the creature
arrived — the check ends where the evidence ends, and the arrival goes to the log. Assert what
...
- **Spawned pals stay.** `pal` asks for a `ChickenPal` in three of its live tests, and each one
...
- `Audio.Handle:setVolume` is actor-wide. It scales everything one actor is emitting rather than
  the sound you called it on, so the tests treat it as a property of the actor.
