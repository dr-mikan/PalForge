...
**No single press runs every check, and the summary says so in words.** 31 checks need a loaded
save and skip at the title screen. Eighteen more are the inverse: they prove a *refusal* path — the
shut ready gate, a widget with no owner to draw under, `Player.coordinate()` with no pawn — so
they can only run where there is nothing to succeed with. Five of those eighteen skip once a world
is up and run at the title screen; the other thirteen need no engine at all and only ever run in a
headless `lua5.4` process. No press in any session state measures everything. Press the key once
at the title screen and once inside a save; a green run is two runs.

You can run the same 612 checks with no game at all, and that is what CI does:
...
**One check separates the two routes, and it is not a discrepancy.** `npm test` calls `run()`
directly; F1 gets there through `install()`, which also binds F1 — so the `ui` check that verifies
*a key PalForge already holds is refused, and the refusal names the holder* has a bound key to be
refused against on the F1 route and nothing to look at on the `npm test` route, where it skips
without a direction. `install()` additionally runs the headless boot bundle first, which reports
its own `8 passed, 0 failed, 0 skipped` line before the 612.

Both figures above are real runs on 2026-08-03 in a plain `lua5.4` process with no engine at all.
In the game the world-gated 31 run instead, and two of the three `could not be answered by this
session` skips go away as well — they exist only because a bare Lua process has no `LoopAsync` and
no UE4SS `Key` table to check against.
...
Startup only **loads** these checks. It never runs them. They spawn pals and hand out items,
so a run only ever happens when you ask for one. The headless `units/` bundle is the one thing
that does run at boot, and that is why it is a separate directory: it touches nothing but Lua
tables.
...
Everything on this page lives under `Scripts/palforge/test/`, and it is one tree with five
corners. `test/init.lua` is the only name production code knows: `core/registry.lua` requires it
under the dev gate and calls `install()`, and that one call runs the boot bundle, loads the case
files, binds F1 and the probe keys, registers every `pf_*` console command including
`ps_catalog`, and hands `core/autorun` its action table.

```text
Scripts/palforge/test/
├── init.lua      the one entry point; install() wires everything below
├── units/        headless suites, run at STARTUP - two files, 8 checks
├── cases/        the in-game API suite, on F1 - 19 files
├── hooks/        measurements that need Palworld running - 25 declared, never auto-run
├── probes/       discovery dumps - not tests; they pass and fail nothing
├── tools/        dev instruments - catalog.lua, the body of ps_catalog
├── support.lua   the world reads, the namespaced ids and the sweep
└── probe.lua     the BEGIN/END bracketing a probe or hook writes its output with
```

| directory | what belongs in it | when it runs |
| --- | --- | --- |
| `units/` | a suite that touches nothing but Lua tables | at startup, every dev session |
| `cases/` | a suite that needs a world, a pawn, a UObject or a UE4SS global, and skips with a direction when its environment is absent | on F1, never by itself |
| `hooks/` | a measurement that cannot be taken with the game switched off, usually with a person watching the screen | only when asked for by name |
| `probes/` | a dump of what the engine actually looks like, so an open question can be closed | on its own key, or as `pf_<name>` |
| `tools/` | an instrument that is not a test at all | when its command is typed |

The rule for `units/` is the narrow one, and it is what keeps startup safe: the whole bundle runs
before the first world exists, in a game that is still loading, so a suite there that blocks or
raises delays every start. If a check needs a world, a pawn, a UObject or a UE4SS global, it is a
`cases/` suite or a `hooks/` measurement instead.

**A release deploy has none of this.** `tools/deploy.sh --release` deletes `palforge/test/` from
the staged copy, all 58 files of it, so the kernel's single `require("palforge.test")` answers
`absent` — the correct state for a player's install rather than an incomplete one. Nothing on this
page reaches a player: no suite, no probe key, no hook, no `pf_*` command and no `ps_catalog`.

## What gets checked

There are **612 checks**, grouped into **19 suites** — a suite is one group of checks,
...
| suite | checks | only inside a save | only with no engine |
| --- | --- | --- | --- |
| `schema` | 28 | 0 | 0 |
| `registry` | 42 | 0 | 0 |
| `definitions` | 23 | 0 | 0 |
...
The middle column is the 31 world-gated checks, counted off a real run. The right-hand column is
the eighteen that cannot run with the game loaded, and it holds two different gates: five call
`support.needNoWorld` and do run at the title screen (`store_api`, `building`, `audio`, `player`,
`events`), while thirteen call `support.needNoEngine` and run only in a headless process
(`store_runtime`'s five, `ui`'s seven and one in `mesh`) — those verify what happens when there is
no `LoadAsset`, no owner and no `FindAllOf` to stub, and a UE4SS session has all three.
...
the hook that measures it. That hook has been run — `pal-skills-equip`, 8 pass / 0 fail on a real
save — so the skip means "measured elsewhere", not "unknown".

The five in `store_runtime` are in neither column either, and for a sharper reason: they drive
the building scan against a fake actor list, and doing that with UE4SS loaded would hand the
real 500 ms sweep a world with none of the player's structures in it — every one of them would
miss and be quarantined. So they run headless under `lua5.4` and skip in the game, in both
states, saying so. The other four store suites are pure Lua and run everywhere. None of the
five writes into `state/`: three substitute the store's I/O entirely, and `store_disk` — the one
that has to meet a real filesystem — works in a throwaway directory under the OS temp directory.
A suite for the module a player's saved state lives in must not be able to write into a player's
saved state.
...
almost nothing, which is why every skip carries a direction now. `31 need a world` is a
sentence you can act on; `35 skipped` is not.
</Callout>
...
`test.run()` with no argument runs those nineteen suites and nothing more. The startup bundle is
a separate set of pure-Lua checks — `palforge.test.units`, run by `test/init.lua`'s `install()`
rather than by the kernel. They register into the same list, and F1 still deliberately leaves
them alone. To reach every registered suite, go one level down:
...
The overlay `tools/deploy.sh` writes lists all nine write opt-ins **commented out**, one per line,
so each is a separate decision. When a whole session is deliberately sweeping them, one flag turns
the lot on and says so:

```bash
tools/deploy.sh --writes    # all nine env.debugHooks opt-ins ON. Throwaway save only
```

It is a separate flag rather than the default because **three of the nine cannot be undone**: a
spawned pal has no per-individual removal, an unlocked technology has no lock — `UPalCheatManager`
declares four unlock entries and no reverse anywhere in the header dump — and a taught active move
is the one write in this tree that has ever correlated with the game closing. (That correlation is
settled now: the target was a villager, not the write. `pal-skills-equip` asks for a
`PalMonsterCharacter` and returned 8 pass / 0 fail with the game still up.)

The twenty-five declared hooks, in the order `pf_hooks_all` runs them — read-only first, so a run
that ends badly has printed everything it could before it got to a write:

| hook | needs | what it settles |
| --- | --- | --- |
| `game-build-live` | world | the raw `GetBuildVersion` / `GetEngineVersion` / `GetGameName` strings, so the declared build can be compared with the running one — **run three times on 2026-08-02**: those three carry *Unreal's* identity and not Palworld's, and the build number comes from `PalGameInstance.DisplayVersion` and `PalUtility.GetDisplayVersion`, which both answer `v1.0.2.101103` |
...
| `mesh-actor-identity` | world | whether a table keyed on a UE4SS actor handle still hits on a later sweep — the measurement the whole per-actor re-key rests on. **Confirmed in three separate sessions on 2026-08-02**: the handle misses, the full name hits, `rawequal` is false and `uo.same` is true, so the key has to be the name |
| `item-datatable-row-read` | world | whether a scalar or FName column can be indexed off the struct `dt:FindRow(id)` returns — **run on 2026-08-02: it can**, and `Item.Handle:recipeOf` reads the game's row because of it |
| `audio-custom-file-loader` | world | whether the shipping build exposes any way at all to turn a `.wav` on disk into something playable — **run on 2026-08-02**: the Wwise external-media half does not exist here, the UE playback half is alive, and what is left is whether a `USoundWave` can be constructed and filled |
...
| `store-base-load-cost` | world | what a real base costs the store to load: the files, their bytes, the decode timed on those bytes, and what the same records would have cost as one file — **run on 2026-08-02: 0 bytes, 0.00 ms, 0 files**, because nothing had registered a building definition |
| `building-actor-streaming` | world, player | whether `FindAllOf("PalBuildObject")` stops returning a base the player has walked away from, and at what distance — **still without a result**: the operator reached 287 m and all 7 structures stayed in every one of the 90 samples, so Palworld's streaming radius is simply larger than that |
| `ui-update-event` | world | arms 21 named UI functions and reports which fire, and in what order — **run on 2026-08-02: three of them do.** `CommonActivatableWidget:ActivateWidget`, `PalHUDService:Push` and `PalHUDService:Close` all fired while the operator opened and closed two screens, which is the rebuild signal `UI.Handle:autoRefresh` now rides. The other 18 stayed silent, and that is an absence of input rather than a finding |
| `pal-spawned-fresh` | world | timestamps every `pal.spawned` firing against `world.ready`, so a fresh spawn can be told from the load storm — **run on 2026-08-02: 27 firings, 17 of them nowhere near a world load**, so the event really does mean a new pal |
| `skill-hit-source` | world | confirms that nothing in the damage path carries a waza id, and quantifies why correlating activation with damage would be wrong — **run on 2026-08-02: not one field in the three damage structs this build declares names a waza** (40 + 6 + 12 fields), which settles `onHit` negatively rather than pending |
...
| `mesh-texture-import-live` | world, player | calls `ImportFileAsTexture2D`, then calls it again for the same path and says whether the cache answered instead of re-importing — **run on 2026-08-02**: a real `Texture2D` came back and the second call was the cached one |
| `mesh-color-change` | world, player, **writes** | puts a tinted mesh in front of the player and changes its colour twice — **run on 2026-08-02, and the operator watched it**: a chest in the air went red → green → blue → gone |
| `audio-setvolume-audible` | world, player | plays one sound at four output-bus volumes so a person can say whether `setVolume` does anything audible — **the one open item.** Run on 2026-08-02: the call returned true at every volume, and the listener reported, twice and hedged, that 0.00 was **not** silent |
...
| `item-satiety-write` | world, player, **writes** | what parameter list the live build declares for `SetFullStomach`, and whether the satiety / HP write is reachable from Lua at all — **run on 2026-08-02: it is.** `SetFullStomach` takes one argument, satiety went 31.648 → 21.648 and was put back, and `AddHPByRate` landed. That measurement is what `Item.Spec.restores` is built on |
| `skill-projectile-spawn` | world, pal, **writes** | prints the declared parameter list of every projectile / spawn route, fires the one that takes no arguments, and refuses the struct ones by name — **run on 2026-08-02: seven parameter lists walked, six routes refused, every one takes a struct.** Settled negative, and shipped as a refusal that carries all six |
| `store-save-roundtrip` | world, **writes** | writes a pack's state through the public surface into a real save's store, reads it back off the real path, and — run again after a reload — across a world load. **Run 1 on 2026-08-02: 7 pass**, 370 bytes written and read back field for field, after it caught a real defect no Linux suite could (`ensureDir` asked `io.open` about a directory; Windows says no). Run 2, the half that crosses a world load, is still owed |
| `store-crash-recovery` | world, **writes** | plants a torn write and an unreadable file in a real save's store and reports which copy the loader recovers from — **run on 2026-08-02: all four rows of the recovery table hold on NTFS**, not only on paper |
| `save-survives-pack-removal` | world, player, **writes** | names every id PalForge made this save record, then — after the pack is uninstalled and the save reloaded — says what became of each. **Started once, never finished**; this is the measurement the whole store pass was begun for |
| `pal-skills-equip` | world, pal, **writes** | whether an active-move write lands on a real pal, and whether the game survives it — **run on 2026-08-02: 8 pass / 0 fail, and the game stayed up.** `AddEquipWaza` equipped `Human_Punch` and it read back off a live `PalMonsterCharacter`, `:forget` took it off, `teachAll(pal)` answered `2, 2`, and `ClearEquipWaza` cleared the loadout with every move restored afterwards |

Nine of the twenty-five declare `writes = true`. `audio-setvolume-audible` deliberately does not:
...
always traceable to something you can actually run. `pf_hooks` lists all twenty-five with the
...
left out of `test.suites()`, so the other eighteen still go.
...
- Press it twice: once at the title screen, once in a save. 31 checks only run in a save and five
  only run without one, so no single press measures everything — and the summary says which.
- `npm test` runs the same 612 checks with no game at all: **576 passed, 0 failed, 36 skipped**.
  Through `install()`, which is the F1 route, it is **577 / 0 / 35** plus the boot bundle's `8/0/0`.
...
- The measurements that need Palworld running are declared hooks under `test/hooks/`: loaded
  ...
  the nine that write into a save need `env.debugHooks[id] = true` on top — `tools/deploy.sh
  --writes` turns all nine on at once, for a throwaway save.
- One tree, five corners — `units/`, `cases/`, `hooks/`, `probes/`, `tools/` — and
  `--release` deletes the lot, so none of it reaches a player.
