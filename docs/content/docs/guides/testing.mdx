...
That last line is worth copying as a habit. `:spawn` places no creature in this version, so the
check asserts exactly that — and if a pal ever does arrive, this test goes red and the failure
is how you find out. A `t:type(ok, "boolean")` on its own would pass either way and tell you
nothing. Assert what the game does today, including the parts that fall short, and let a change
...
all report numbers you can subtract, so a test can say what actually moved instead of only that
...
<Callout type="info">
`:spawn` places no creature in this version, so the pal suite checks the `false` and then checks
the measurement that verdict is built on: `nearestPal` proves the world can still be enumerated
around the point that was asked for, which is what a working spawn would be detected by.
</Callout>
...
- **No pal is spawned.** `pal` asks `UPalCheatManager` for a `ChickenPal` in three of its live
  tests, and no creature appears in this version, so those three leave nothing behind. Should
  one ever arrive, it stays: nothing in the tree can despawn a pal.
- **Given Wood is handed straight back, onto the floor.** `item` gives three `Wood` and takes
  the same three back. Taking drops them on the ground where you are standing rather than
  destroying them, so what a run leaves behind is a small pile at your feet — and if the give
  landed and the take did not, the Wood is in your inventory instead.
...
<Callout type="warn">
Run the world-touching suites in a throwaway save, not the one you care about. Three Wood on the
floor is a small mess, but it is a mess, and the framework makes no promise to clean it up.
</Callout>

A green run does not mean every call in the api does the whole job its name suggests. Three
of them fall short today, and the suites pin that down on purpose:

- `Pal.Handle:spawn` places no creature in the world. The three live pal checks assert the
  `false` rather than skipping past it, so the day one appears the suite says so.
- `Audio.Handle:setVolume` returns `false` for every value, including the edges. It changes
  nothing, and this build has no per-sound volume for it to change. Pick a quieter sound event,
  or leave the loudness to the game's own audio options, and treat the `false` as the honest
  answer.
...
- Run the world-touching suites in a throwaway save: what a live check gives you stays, and
  what it takes back lands on the floor.
