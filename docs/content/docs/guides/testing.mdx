...
save and skip at the title screen. Eleven more are the inverse: they prove a *refusal* path — the
...
process with no engine at all. In the game the world-gated 28 run instead, and two of the three
`could not be answered by this session` skips go away as well — they exist only because a bare
...
There are **580 checks**, grouped into **19 suites** — a suite is one group of checks,
...
sentence you can act on; `32 skipped` is not.
</Callout>
...
| `store-base-load-cost` | world | what a real base costs the store to load: the files, their bytes, the decode timed on those bytes, and what the same records would have cost as one file |
| `building-actor-streaming` | world, player | whether `FindAllOf("PalBuildObject")` stops returning a base the player has walked away from, and at what distance |
...
| `store-save-roundtrip` | world, **writes** | writes a pack's state through the public surface into a real save's store, reads it back off the real path, and — run again after a reload — across a world load |
| `store-crash-recovery` | world, **writes** | plants a torn write and an unreadable file in a real save's store and reports which copy the loader recovers from |
| `save-survives-pack-removal` | world, player, **writes** | names every id PalForge made this save record, then — after the pack is uninstalled and the save reloaded — says what became of each |
...
- The measurements that need Palworld running are declared hooks under `test/hooks/`: loaded
  ...
  the eight that write into a save need `env.debugHooks[id] = true` on top.
