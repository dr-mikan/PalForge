...
env.debug = true    -- さらに test/hooks 配下の、ゲームが必要な 19 のテストフック
```
...
このファイルが持つのは**名前の一覧であり、コードではありません**。各行は
`require("palforge.test").ACTIONS` で引かれ、一致しない名前は報告のうえスキップされます。
...
`core/autorun.lua` の変更は不要です。そして迷い込んだファイルが、この MOD がキー操作でできないことを
実行することもありません。セーブに書き込むフックには、その上でさらに専用ゲート
...
- `autorun.txt` は 1 行 `[delay] name` で、`palforge.test` の `ACTIONS` に照合され、ワールドのロードごとに 1 回走ります。
