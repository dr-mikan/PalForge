...
- Play a sound, hand out a buff, call a pal and pay out an item from that one interaction
...
- Calling a pal goes through `UPalCheatManager`, the game's own admin object. PalForge finds
  one, or builds one on the player controller itself, so you only need to be in a world. With
  no controller yet the spawn logs a warning and returns `false`, and everything else in the
  pack still runs.
- Handing out an item needs none of that. `Item.Handle:give` goes through the inventory's own
  `AddItem_ServerInternal` call.
...
<Callout type="warn">
Two things here do not work yet, so plan around them. `Handle:setVolume` returns `false` and
changes nothing — pick a sound event that is already the loudness you want. A `soundFile`
(your own audio file) is accepted and then does nothing, so use a game `AkAudioEvent` from
the catalog instead. `Handle:stop` is not per sound either: it stops everything playing on
that actor.
</Callout>
...
<Callout type="warn">
A PalForge effect does not switch on the game's own status icons. `EPalStatusEffectType` is
one of the game's internal lists, and no call to apply one is confirmed, so `nativeStatus` is
a label you can read back rather than something that acts. Put whatever the effect should
actually do into `onTick`.
</Callout>
...
[PalForge.spawn][info] spawn.palAt Kitsunebi (lv 5) accepted; relocation to (12345,-6789,420) scheduled (SpawnMonster+teleport)
...
of the lines is not. `onActivate` runs the sound, the effect and the spawn before it logs its
own line, so the blessing and the spawn are reported first — and `give Ruby x3` only after
...
| Pal | `onSpawned`, `onDamaged`, `onDeath`, `onCaptured`, `onTick` | `:spawn()`, `:renderOn(actor)`, `:skillsOf()` |
| Item | `onObtain`, `onUse` | `:give(n)`, `:take(n)`, `:iconOf()`, `:recipeOf()` |
...
**The pal does not appear.** A coordinate spawn needs a player controller in the session.
Without one it logs
`spawn.palAt: no PalCheatManager and none could be constructed (no player controller yet?)`
and returns `false`; a `:spawn()` with no `at` reports the same thing as `spawn.pal:`. A
`true` means the spawn was accepted and the move to your coordinate was scheduled — not that
a pal is standing there yet. The rest is reported in the log.
