...
**Write your pack against `PalForge.pack("beacon")` rather than against `api` directly.** It
hands back the same nine members, with the eight constructors wrapped so that every definition
made through them records `beacon` as its owner. That ownership is what turns an id collision
from a silent overwrite into a log line naming both packs, and it is the only way the framework
can attribute a definition at all — a define call is a plain Lua call and carries no evidence of
who made it. A pack that skips it still works; its content simply registers with no owner.
...
`depends` (and `recommends`) record the namespaces this pack may mention in a value, so an id
from someone else's namespace is either declared or reported. `Player` is the one member that
is not wrapped: it defines nothing, so there is nothing to attribute.
...
<Callout type="warn">
`soundFile` — an audio file of your own — is refused at define time, with an error naming the
open item `audio-custom-file-loader`. Custom audio does not play on this build: the UE-native
route has no importer that survives shipping, and the Wwise route rebinds media the cook already
staged rather than reading a file. Name a game sound instead. And neither `Handle:stop` nor
`Handle:setVolume` is per sound: both act on everything playing on that actor, so to make one
sound quieter than another, pick a quieter event.
</Callout>
...
<Callout type="info">
All four item handlers are live. `onObtain` has two sources, deduped into one event:
the game's own "you obtained an item" log (`PalPlayerState:AddItemGetLog_ToClient`,
`ctx.via = "getlog"`) and the inventory add (`PalPlayerInventoryData:AddItem_ServerInternal`,
`ctx.via = "additem"`) — both are armed because they fail in opposite directions, and `ctx.via`
says which one carried this one. `onCraft` fires when a production finishes at a real machine
(`ctx.itemId`, `ctx.recipeId`, `ctx.via = "convert" | "product"`); `ctx.count` is deliberately
absent, because the per-craft count lives in the recipe row and a hook is no place for a data
table read. `onDiscard` fires on a drop or a dispose (`ctx.itemId`, `ctx.count`,
`ctx.reason = "drop" | "dispose"`).
</Callout>
...
| Item | `onObtain`, `onUse`, `onCraft`, `onDiscard` | `:count()`, `:give(n)`, `:take(n)` (consumes; needs the player to have something equipped), `:iconOf()`, `:recipeOf()` |
...
Two handlers can be declared and never run: `Building.onLeftClick` and `Building.onBreak`.
That is a settled negative rather than a gap yet to be filled — the complete function lists of
every class that could own one were read. `PalBuildObject` has 22 functions and no click, hit
or strike entry; the one damage-shaped entry fires on a 12-13 second deterioration timer with
no player anywhere near the structure; and destruction exists only as delegate *fields*, which
`RegisterHook` cannot address by path. Use `onRightClick` for interaction, and `onRemove` with
`ctx.reason = "missing"` for a structure that disappeared — which cannot tell a dismantle from
a streamed-out chunk. Both stay declarable so a pack's own emit works and a future source has
somewhere to arrive.
...
`Pal.onSpawned` is armed on three sources, all of them only after the world has loaded — the
same signal fires in the load storm for every pal already in the world, and arming it at
startup wedged the shared hook dispatch. `BroadcastOnCompleteInitializeParameter` was measured
carrying nothing in a real save, so the two that do the work are the delegate targets bound to
it: `PalPlayerCharacter:OnCompleteInitializeParameter` (which only fires for characters the
player subscribed to) and `PalNPC:OnCompletedInitParam` (bound on the pal's own side, so it
does not depend on anyone subscribing). Keep the handler safe to run twice.
...
That call needs a `PalCheatManager` to already exist in the session, so it wants the
CheatManagerEnabler mod — unlike the spawn route, which builds one on the player controller
itself when there is none. The unlock goes through the item helper's own lookup, and that one
asks for a cheat manager rather than constructing one. It also verifies afterwards: a technology
row of that name has to exist, or the log says the call was issued with nothing to unlock and
`unlock()` answers `false`.
...
And the id's own shape, checked at define time. A namespaced id resolves to the data table row
name `packid_name`, so both halves have to be letters, digits or `_` — a hyphen would define and
register and then never resolve to anything:
...
**Two definitions, one id.** The last definition to run still wins, but the registry says so
now instead of overwriting in silence — and if the two came from different packs it names both:
`item 'mypack:Potion' was defined by pack 'mypack' and is being redefined by pack 'other'; the
new definition replaces the old one (last-wins)`. That line only exists when a pack said who it
was, which is what `PalForge.pack("beacon")` is for. Define at load, and reach for `X.get(id)`
inside handlers rather than redefining on every event.
...
- Take the api from `PalForge.pack("beacon")`, not from `PalForge.api`: it is the same nine
  members, and every definition made through it records which pack owns it.
- `id` is the only required field anywhere, and its shape is checked when you define. An id
  with a colon (`"beacon:Flare"`) is your own; an id without one (`"Altar"`, `"Ruby"`,
  `"Kitsunebi"`) is content the game already has. Both halves are letters, digits or `_`.
