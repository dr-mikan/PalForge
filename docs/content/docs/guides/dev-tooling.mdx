...
env.debug = true    -- additionally: the twenty-five game-required test hooks under test/hooks
```
...
The file holds **a list of names, never code**. `core/autorun.lua` names nothing under `test/`:
`test/init.lua`'s `install()` hands it the table with `autorun.setActions(M.ACTIONS)`, each line is
a lookup in what it was handed, and a name with no match is reported and skipped:
...
with no change in `core/autorun.lua` — and with no test tree at all, which is what `--release`
deploys, there is simply no table and the queue is a no-op that costs nothing. A stray file cannot
execute anything the mod does not already do on a keypress. A hook that writes into a save still carries its own second gate
...
- `autorun.txt` is `[delay] name` per line, resolved against the `ACTIONS` table `test/init.lua` hands `core/autorun`, once per world load.
