...
env.debug = true    -- 另外加载 test/hooks 下那二十四个需要游戏在跑的测试钩子
```
...
文件里装的是**一串名字，不是代码**。`core/autorun.lua` 不写任何 `test/` 底下的名字：`test/init.lua`
的 `install()` 用 `autorun.setActions(M.ACTIONS)` 把表交给它，每一行只在这张交来的表里查一次，查不到
的会被报告并跳过：
...
于是测试面里任何地方注册的命令，从它存在的那一刻起就能从这里跑，不需要改 `core/autorun.lua`；而在
`--release` 部署出来的那种没有测试树的副本里，这张表根本不存在，队列就是一个零成本的空操作。一个
...
- `autorun.txt` 每行一个 `[delay] name`，对照 `test/init.lua` 交给 `core/autorun` 的 `ACTIONS` 解析，每次载入世界跑一遍。
