...
| 贴图 | **可以** — 磁盘上的 PNG，导入后按路径缓存 | 可以 |
| 声音 | **不行** | 可以 — 一份 1957 条事件名的目录 |
| 材质 | **不行** | 可以 — 把动态实例挂到已加载的材质上 |

那两行「不行」说的是引擎，不是框架。这些路线并不是写了一半，而是根本没有可调用的入口。

所以今天的 PalForge 是一个**复用游戏自身资源**的框架，外加两条都被亲眼看着跑通的磁盘路线（OBJ
几何体和 PNG 贴图），另有三行是根本没有入口可调的。在开始建模之前，这件事值得先知道。
...
**2026-08-02 在一次加载好的存档里，这个调用真的跑了，而且答了。** pack 自带的 PNG 返回的是一个
真正的 `Texture2D`（类链是 `Texture2D : Texture : StreamableRenderAsset : Object`，所以它满足材质
那个 setter 声明的 `UTexture*`），同一路径的第二次调用返回了完全相同的值 —— 这只有缓存做得到。
在此之前刚刚失败过的那条路径，等文件写好之后再调用也成功了：失败不会被记住。

这条路线的代价是：每条路径、每个会话会分配一个 `UTexture2D`，并且永远不会被释放 —— UE4SS 的 Lua
层没有销毁调用，这个进程里的任何东西都放不掉它。缓存真正的作用就在这里：一个网格的贴图哪怕每只
帕鲁生成时都要挂一次，也只导入一次。
...
`Normal Map` 等），写入调用也和声明完全一致；而 2026-08-02 `mesh-color-change` 跑过一次，有人
盯着屏幕：一个箱子依次变成**红 → 绿 → 蓝**。整条路线已经从头到尾都测过了。
</Callout>
...
- PNG 贴图能导入，并按路径缓存；每多一条路径，就多一张活到会话结束的贴图。
