...
最後の行は習慣として真似する価値があります。このバージョンでは `:spawn` は何も配置しないので、
チェックはまさにそのとおりを主張します。もしパルが現れるようになれば、このテストが赤くなり、
それが知らせになります。`t:type(ok, "boolean")` だけならどちらの世界でも通ってしまい、何も
教えてくれません。ゲームが今日していることを、足りない部分も含めてそのまま主張し、変化には
意図的にテストを壊させてください。
...
<Callout type="info">
このバージョンでは `:spawn` は何も配置しないので、pal スイートはまず `false` を確かめ、次にその判定の
土台になっている計測そのものを確かめます。`nearestPal` は、要求した地点のまわりで今もワールドを
列挙できることを示します。それこそが、動くスポーンを検出する手段だからです。
</Callout>
...
- **パルはスポーンしません。** `pal` はライブテスト 3 件で `UPalCheatManager` に `ChickenPal` を
  要求しますが、このバージョンでは何も現れないので、この 3 件は何も残しません。もし現れるように
  なった場合は残ります。ツリー内にパルをデスポーンさせるものはありません。
- **渡した Wood はそのまま返され、床に落ちます。** `item` は `Wood` を 3 個渡し、同じ 3 個を取り戻します。
  取り出しは削除ではなく、立っている場所の地面に落とすので、実行が残すのは足元の小さな山です。
  give が成立して take が成立しなかった場合は、代わりにインベントリに残ります。
...
<Callout type="warn">
ワールドに触れるスイートは、大事なセーブではなく使い捨てのセーブで実行してください。床の Wood 3 個は
小さな散らかりですが、散らかりであることに変わりはなく、フレームワークはそれを片付ける
約束をしていません。
</Callout>

緑の実行結果は、api のすべての呼び出しが名前どおりの仕事を最後までやる、という意味ではありません。
いまのところ 3 つは途中までしかやらず、スイートはそれを意図的に固定しています。

- `Pal.Handle:spawn` はワールドにクリーチャーを配置しません。ライブの pal チェック 3 件は、素通り
  させずに `false` を主張するので、現れるようになった日にスイートがそれを知らせます。
- `Audio.Handle:setVolume` は端の値も含め、どんな値に対しても `false` を返します。何も変わらず、
  このビルドにはサウンド単位の音量そのものがありません。より静かなサウンドイベントを選ぶか、音量は
  ゲーム自身のオーディオ設定に任せ、`false` は正直な返答として受け取ってください。
- `Audio.Handle:stop` はアクター単位です。`StopSoundByActor` を発行するので、呼び出した対象の音 1 つ
...
- ワールドに触れるスイートは使い捨てのセーブで実行します。ライブチェックが渡したものは残り、
  取り戻したものは床に落ちます。
