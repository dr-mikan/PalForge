...
宣言済みの 13 個のフックを、`pf_hooks_all` が実行する順（読み取り専用が先。途中で落ちても、書き込みに
...
13 個のうち 3 個が `writes = true` を宣言します。`audio-setvolume-audible` は意図的に宣言しません。
音は鳴りますが、変えたものはセッションを越えず、ファイルにも届きません。`writes` は「何かが起きる」
ではなく「セーブが書き換わる」という意味だからです。
...
つまりスキップは常に、実際に実行できる何かへ辿れます。`pf_hooks` は 13 個すべてを、いま
スキップされる理由付きで一覧にします。
</Callout>
