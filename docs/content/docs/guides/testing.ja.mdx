...
<Callout type="info">
`:spawn` が `true` を返すのは**受理された**という意味であり、到着したという意味ではありません。
アクターは数フレーム後に実体化し、再配置はリトライチェーン上で走るため、pal スイートは要求座標との
距離をあえて確かめません。確かめるのはスポーンが受理されたことと、ワールドが列挙できることだけです。
</Callout>
...
- **スポーンしたパルは残ります。** `pal` はライブテスト 3 件で `UPalCheatManager` に `ChickenPal` を
  要求します。ツリー内にそれをデスポーンさせるものはありません。
- **渡したアイテムも残ることがあります。** `item` は `Wood` を 3 個渡し、直後に `:take(3)` を呼びます。
  このビルドでは確認された削除呼び出しがないため、`:take` は同じ追加経路に負のデルタを流し込んで
  個数を読み直します。テストが主張するのは、実測した真偽値を返すことだけです。`false` は 3 個の Wood
  がまだインベントリにあるという意味です。
...
- **エフェクトは自分で後始末します。** ライブの effect テストは 1 件だけで、`pcall` の下でポーンに
  適用してから外します。PalForge のエフェクトは純粋な Lua の帳簿であり、いずれにせよネイティブの
  状態異常は切り替わりません。

<Callout type="warn">
ワールドに触れるスイートは、大事なセーブではなく使い捨てのセーブで実行してください。チキン 3 羽と
Wood 3 個は小さな散らかりですが、散らかりであることに変わりはなく、フレームワークはそれを片付ける
約束をしていません。
</Callout>

緑の実行結果は、api のすべての呼び出しが名前どおりの仕事を最後までやる、という意味ではありません。
いまのところ 2 つは途中までしかやらず、スイートはそれを意図的に固定しています。

- `Audio.Handle:setVolume` は端の値も含め、どんな値に対しても `false` を返します。まだ何もしません。
  音量はゲーム自身のオーディオ設定で調整し、`false` は正直な返答として受け取ってください。
...
- ワールドに触れるスイートは使い捨てのセーブで実行します。出したパルや渡した Wood は残ります。
