...
这五个都是你自己的记账，不会告诉游戏任何事。另外三个 —— `:teach`、`:forget` 和 `:skillsOn` ——
正好相反：它们往真正的角色上写，也从角色上读回来。下面有专门的一节。
...
整张表是 `nil` 则表示这个角色根本读不出来 —— 是“未知”，不是“没有” —— 所以取下标之前先判断 `nil`。
...
<Callout type="info">
要问就问**帕鲁**，别问村民或商人。它们也是角色，游戏自己的角色列表里也有它们，但它们身上没有技能 ——
所以问它们得到的是空列表，和一次失败的读取长得一模一样。
</Callout>
...
<Callout type="warn">
别假定，去看那个布尔值。读配置这件事已经定下来了，写配置还没有到可以拿来搭整合包的程度；`:teach`
返回 `false` 是框架在如实告诉你，而不是你的代码有 bug。在真正依赖它之前，先在一个可以重来的存档里
试一次。
</Callout>
...
<Callout type="warn">
这个数组就只是一个列表。光是声明它，不会往任何生物身上装上什么，游戏也不会替你发动那些主动技能。
它只是被存下来，通过 `pal:skillsOf()` 交还给你。
</Callout>
...
- `pal:skillsOf()` 给出的是一只帕鲁*声明*的 id。`skill:teach(actor)` 把一个技能装到活着的角色身上，
  `skill:skillsOn(actor)` 读回那个角色真正带着什么，`pal:teachAll(actor)` 一次装上整份声明列表。
