...
- **主动技能的写入没有测过。** `AddEquipWaza` / `RemoveEquipWaza` / `ClearEquipWaza` 从来没有被看到成功
  装到一只帕鲁身上。唯一一次真的写入之后 1.4 秒，Palworld 就关闭了；而那次用的是过宽的 `PalCharacter`
  检索，目标很可能是个 NPC —— 所以“写错了对象”是目前最有力的解释，但那不是可以拿来发布的事实。

<Callout type="warn">
装上**主动技能**是这里唯一还没被测量的能力，对应未决项目 `pal-skills-equip`。别假定，去看那个布尔值，
并且在一个丢了也不心疼的存档里试。这个测量不是框架自己会做的事，而是一个声明好的测试钩子：在 UE4SS
控制台里运行 `pf_hook pal-skills-equip`，它需要 `env.debug`，而且因为它会写进真实存档，还需要
`env.debugHooks["pal-skills-equip"]`。读配置和写被动都已定下来，没定下来的只有这一件。
</Callout>
...
- `pal:skillsOf()` 给出的是一只帕鲁*声明*的 id。`skill:skillsOn(actor)` 读回活着的角色真正带着的那四个
  列表，`skill:teach(actor)` 装上一个（被动已定下来，主动技能仍未测量），`pal:teachAll(actor)` 一次装上
  整份声明列表。
