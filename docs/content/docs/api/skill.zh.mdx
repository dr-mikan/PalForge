...
每一次写入之后都会把角色读回来核对，所以 `true` 表示这个技能**在角色身上被看到了** —— 绝不是“调用
没报错”。`false` 表示它没有装上：目标不是角色、游戏不接受这个 id，或者写入被拒绝了。

`:skillsOn(actor)` 返回 `{ active = {...}, passive = {...} }`。空列表表示读成功了、这个角色一个都
没有。整张表是 `nil` 则表示这个角色根本读不出来 —— 是“未知”，不是“没有” —— 所以取下标之前先判断
`nil`。
