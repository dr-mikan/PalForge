...
<Callout type="warn">
パックのために `refresh()` を呼んでくれるものはありません。どの要素も、自分の状態が変わった
ときに自分で `:refresh()` を呼ぶか、`:autoRefresh(ms)` を選ぶかのどちらかです。**そして
`autoRefresh` が乗っているのは 1 つではなく 2 つです。**このページの以前の版は、PalForge に
あるリフレッシュの駆動手段はポーリングだけだと書いていました。それは同じ日の計測でくつがえり、
その訂正がここです。

- **ゲーム自身の再構築シグナル。**`CommonActivatableWidget::ActivateWidget`、
  `PalHUDService::Push`、`PalHUDService::Close` はいずれも、Palworld が画面を組み立てるとき・
  畳むときに発火します。2026-08-02 にパス指定で 21 個の候補を仕掛け、そのうち 3 つが発火しました。
  `native/ui/refresh.lua` はこの 3 つを、遅延して、1 セッションにつき最大 1 回だけ仕掛けます。
  再構築から心拍 1 つ以内にリフレッシュが届きます。
- **その下で床になっているのが心拍です。**これは外せませんし、フォールバックでもありません。
  その実行で残り 18 個の候補は沈黙しましたが、それは*入力*が無かったということです。操作者が
  開いた画面は 18 枚ではなく 2 枚でした。つまり、あの 3 つが拾わない再構築こそ、この床が
  存在する理由です。シグナルは `world.ready` の後にしか仕掛けられません。UE4SS はフックを外せず、
  `ActivateWidget` はワールドロードの嵐の最中にすべての activatable に対して発火するからです。
  したがってタイトル画面の要素はポーリングだけで動きます。

つまりパネルは、**何が起きても最大 `ms` ミリ秒だけ古い内容を表示し、画面の再構築後はおよそ
心拍 1 つぶん古い内容を表示します**。前者を動かすてこは、いまも `ms` を短くすることだけです。
何も変わっていないパネルに対する 1 拍は意図的に安上がりです。バインディングは比較し、変わった
ものだけ書き戻すので、`ms` を短くしてもコストはネイティブ呼び出しではなく比較であり、シグナルに
乗ることが床を遅くすることもありません。
</Callout>
...
方法その 2 — 購読する。どちらの購読も `event.every` に乗り、`event.TICK_MS`（500 ms）の心拍を
丸ごと数えるので、床の実際の周期は 500 の倍数に切り上がります。再構築シグナルは、仕掛かっている
ときはその拍を置き換えるのではなく、上にリフレッシュを 1 回足します。
...
<Callout type="info">
**このページを信じるより、実際に何がリフレッシュを駆動しているかを尋ねてください。**
`UI.refreshDriver(ms)` はいま仕掛かっているものを読み、答えをデータとして返します。
...
`kind == "poll"` は失敗を意味しません。シグナルが存在する前に出荷されていたのがこれであり、
タイトル画面でも、仕掛けが断られたセッションでも、`RegisterHook` そのものが無いヘッドレスの
テストでも、これが正直な答えです。`kind` で分岐してよいのは、読み出しがどれだけ古くなりうるかを
*言う*ときだけで、リフレッシュが効くかどうかの判断に使ってはいけません。どの答えの下にも床が
あります。

計測そのものは `pf_hook ui-update-event` で、これは再実行できます。沈黙した 18 個の候補
——ポップアップ用の `PalHUDService:ShowCommonUI`、タイトルへ戻るときの `RemoveHUD`、
ワールド HUD の 2 つ、CommonUI コンテナの追加／削除——は、それらを動かすような操作を与えられて
いなかっただけなので、切り捨てずにフックの中に名前を残してあります。
</Callout>
...
| `:autoRefresh(ms)` | `boolean` | Palworld の再構築シグナルと、床としての心拍の両方に乗せて `refresh()` を駆動。`ms` の既定は 500 |
| `:autoMount(root, ms)` | `boolean` | 同じ駆動でライフサイクル全体を見る。降りていれば `mount` を再試行、上がっていればリフレッシュ。`ms` の既定は 2000 |
...
| `UI.refreshDriver(ms)` | `table` | いま実際にパネルをリフレッシュしているもの。`kind`、`event`、`state`、`staleMs`、`why` |
...
- リフレッシュを駆動するのは 2 つです。Palworld 自身の再構築シグナルと、その下で床として
  効き続けるハートビート。`:autoMount(root, ms)` が生きている PalForge の UI がすべて通る
  マウント経路で、パネルは最大でも `ms` だけ古くなり、いまどちらが実際に武装されているかは
  `UI.refreshDriver(ms)` が答えます。
