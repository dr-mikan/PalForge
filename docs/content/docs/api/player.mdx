...
<Callout type="warn">
A coordinate spawn is not instant, and it is a spawn followed by a move. The game's own call
drops the pal beside the player, and PalForge waits for it to appear and then relocates it onto
your point — where it lands exactly. All of that takes four to eight seconds, so what you see is
the pal turning up next to you and then moving.

`:spawn` returns before any of it has happened, so the boolean says the call was issued and
nothing more. See [Pal](/docs/api/pal).

It does **not** need the `CheatManagerEnabler` mod. `core/spawn` looks for a live
`PalCheatManager`, then for one on the player controller, and when the session has neither it
builds one itself: `StaticConstructObject` on the `PalPlayerController`'s own `CheatClass`,
falling back to `/Script/Pal.PalCheatManager` and then `/Script/Engine.CheatManager`, attached to
the controller so it happens once per session rather than once per spawn. What a world spawn does
need is a player controller — no world loaded, or not connected yet, and it answers `false`
before reaching the game and warns that none could be built.
</Callout>
...
The local player's inventory is readable, and that read is the one item route on this build
measured end to end. `Item.Handle:count()` walks the `PalUtility` CDO to the player, to the
player state, to `InventoryData`, and asks the game's own `CountItemNum`: a loaded save answered
`135` for `Wood` as a plain Lua number, with a real object printed at every step of that chain.
...
`nil` means **unknown**, never zero — no world, no player, or a read that would not answer. Guard
it exactly the way you guard `Player.coordinate()`.

<Callout type="warn">
Reaching past the API and calling `CountItemNum` yourself is fine, but the id must be wrapped:
`inv:CountItemNum(FName("Wood"))` answers, while `inv:CountItemNum("Wood")` — a bare Lua string
where an `FName` is declared — faults inside UE4SS's argument marshalling and takes Palworld down,
and `pcall` never sees it. That happened once, on the line after the read that succeeded. It is
also why the 64-bit sibling `CountItemNum64` is called nowhere in PalForge: it is in the class's
function list, its parameter list has never been read, and guessing at a declaration is what cost
the session.
</Callout>
...
- What the player is holding is readable: `Item.get(id):count()`, with `nil` meaning unknown.
