...
nothing more. See [Pal](/docs/api/pal). It also needs the `CheatManagerEnabler` mod: without it
there is no admin object at all, and a world spawn answers `false` before it reaches the game.
</Callout>
