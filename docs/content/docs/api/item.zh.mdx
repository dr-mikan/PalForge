...
| `:take(count)` | boolean | 把 `count` 个（默认 1）丢在玩家脚边，从背包里移除；`count` 按绝对值处理。只有观察到数量确实下降时才是 `true`。 |
...
`:give` 通过游戏自己的管理对象把道具放进玩家背包，然后核对它们是不是真的到了：调用之前读一次背包数量，调用之后再读一次，`true` 表示看到数量**上升**了。
...
<Callout type="warn">
`:give` 和 `:take` 都要走游戏的管理对象，而打开它的是配套模组 `CheatManagerEnablerMod`。没有它就没有可调用的对象，两者都会在够到游戏之前被拒绝并返回 `false`。要写进 `mods.txt` 的那一行见 [快速开始](/docs/getting-started)。
</Callout>

这个布尔值是实测结果，不是“调用成功了没有”。`false` 表示没有观察到东西到位，原因可能有好几个：没有管理对象、游戏不认识这个道具 ID、背包已经放不下。每一种结果都会连同道具名、数量和背包的变化写进日志，所以布尔值分不出来的情况，日志能分出来：
...
带命名空间的 ID 会先被解析，所以 `Item.get("example:Potion"):give(1)` 交给游戏的名字是 `example_Potion`。PalForge 不会检查对应的数据行是否存在 —— 游戏不认识的 ID 也照样被接受、什么都没加，然后以 `false` 的形式回来。

<Callout type="info">
别指望自己调用的 `:give` 会回到自己的 `onObtain`。`onObtain` 跟的是游戏自己的拾取汇报，而且在 `onObtain` 处理函数里面还有一道保护，不让处理函数把自己再触发一遍。你自己发出了什么，就在发出的那个位置写进日志。
</Callout>

### take

`:take` 是同一套实测，方向相反：前后各读一次数量，`true` 表示看到数量**下降**了。

<Callout type="warn" title="take 会把道具丢在玩家脚边">
这个版本上没有任何调用能让道具凭空消失，所以 `:take` 用的是把它从背包里取出来的那个调用：道具会掉在玩家站着的地面上。背包确实少了它们 —— 这正是 `:take` 的含义，数量核对也证实了 —— 但玩家旁边会出现一堆东西，谁都能走过去把它捡回来。

请按“会留下一堆”来设计。`:take` 适合“把东西交出来”，不适合“把东西消耗掉”，所以只有在玩家原地捡回去也不会破坏设计时，才用它来收取代价。
</Callout>
...
<Callout type="warn">
被夹过的移除照样返回 `true` —— 上面第二行要 5 个、只丢了 2 个，返回的仍是 `true`。`true` 的意思是“看到数量下降了”，不是“正好走了 `count` 个”。数量必须精确时，自己在前后各读一次 `:count()`。
</Callout>
...
先扣、扣成功了才给：这个顺序才能让补给台不至于白发东西，因为没看到东西离开时 `:take` 会返回 `false`。但有两点它不保证。木头不会消失 —— 它就在玩家旁边的地上，可以直接捡回去，所以这个台子是图省事的交换，而不是真正的代价。另外 `true` 也不表示 5 个全走了：请求会被夹到玩家实际带着的数量，所以必须精确的交易还是得自己查一次 `:count()`。
...
**拿走的道具会掉在玩家脚边。** 这个版本上没有销毁道具的手段，所以 `:take` 是把它们从背包移到世界里。移除本身是真的，也是实测过的；地上那一堆同样是真的。
...
**`give` 和 `take` 需要管理对象。** 两者都要走 `CheatManagerEnablerMod` 打开的那个对象。没有这个配套模组，它们在够到游戏之前就被拒绝，返回 `false`。`count` 不需要它。

**别指望一次 `:give` 会回到你的 `onObtain`。** 你自己的代码发出了什么，就在调用的那个位置写进日志，别等事件绕回来。
...
- `Item.get("Wood")` 会给你任意道具的句柄。`:count()` 读出玩家现在带着多少，`:give` 往里加，`:take` 往外拿 —— 三者返回的都是“实测到了什么”，而不是“试着做了什么”。`:take` 拿走的东西会掉在地上。
