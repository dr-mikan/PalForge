...
<Callout type="warn">
没有任何东西会替整合包调 `refresh()`。一个元素要么在自己状态变化时自己调 `:refresh()`，要么选择 `:autoRefresh(ms)` —— **而 `autoRefresh` 搭乘的是两样东西，不是一样。** 本页早先的版本说过轮询是 PalForge 唯一的驱动；那个说法在同一天被一次测量推翻了，更正就写在这里。

- **游戏自己的重建信号。** Palworld 建起或拆掉一个画面时，`CommonActivatableWidget::ActivateWidget`、`PalHUDService::Push` 和 `PalHUDService::Close` 都会触发 —— 2026-08-02 按路径布上了 21 个候选，其中三个触发了。`native/ui/refresh.lua` 会给这三个布钩，惰性地、每次会话至多一次，而刷新会在一次重建之后一拍之内落地。
- **底下那层心跳，作为下限。** 它拆不掉，也不是备用方案。那次运行里 21 个候选中有 18 个一直没有声音，而那是缺少*输入*—— 操作者打开的是两个画面，不是十八个 —— 所以这三个盖不到的重建，恰恰就是这条下限存在的理由。而且这个信号只在 `world.ready` 之后才布上，因为 UE4SS 没法注销一个钩子，而 `ActivateWidget` 会在世界加载风暴里为每一个 activatable 触发；所以标题画面上的元素只靠轮询跑。

于是一块面板**无论如何最多旧 `ms` 毫秒，而在一次画面重建之后大约旧一拍**。把 `ms` 调小，仍然是对前一个数字唯一的杠杆。对一块闲着的面板来说，一拍是特意做得很便宜的 —— 绑定只做比较，只写回变了的东西 —— 所以 `ms` 调快，代价是比较，不是原生调用，而搭上这个信号也不会把下限拖慢。
</Callout>
...
办法二 —— 订阅。两个订阅都搭在 `event.every` 上，它按整拍数 `event.TICK_MS`（500 ms）计数，所以这条下限的真实周期会向上取整到 500 的倍数；而重建信号在布上的时候，是在这个拍子之外再加一次刷新，而不是取代它。
...
<Callout type="info">
**别信这一页，直接问现在到底是什么在驱动刷新。** `UI.refreshDriver(ms)` 会读取实时的布钩状态，把答案作为数据交回来：
...
`kind == "poll"` 从来都不是故障状态 —— 它是这个信号出现之前就在跑的东西，而且在标题画面上、在布钩被拒绝的会话里、在根本没有 `RegisterHook` 的无头测试里，它都是诚实的答案。只用 `kind` 去*说明*一份读数可能有多旧，绝不要用它来判断刷新还能不能工作：下限在每一个答案底下都在。

测量本身是 `pf_hook ui-update-event`，而且它可以再跑：那 18 个一直没有声音的候选 —— 弹窗用的 `PalHUDService:ShowCommonUI`、回标题用的 `RemoveHUD`、世界 HUD 那一对、CommonUI 容器的 add/remove —— 从来就没有被给过能触动它们的动作，所以它们仍然点名留在这个钩子里，而不是被划掉。
</Callout>
...
| `:autoRefresh(ms)` | `boolean` | 让 `refresh()` 同时搭乘 Palworld 的重建信号和作为下限的心跳；`ms` 默认是 500 |
| `:autoMount(root, ms)` | `boolean` | 整个生命周期都搭在同一个驱动上：下来时重试 `mount`，上去后刷新；`ms` 默认是 2000 |
...
| `UI.refreshDriver(ms)` | `table` | 此刻究竟是什么在刷新一块面板：`kind`、`event`、`state`、`staleMs`、`why` |
...
- 刷新搭乘两个驱动：Palworld 自己的画面重建信号，以及在它下面兜底的心跳。`:autoMount(root, ms)` 是每个活着的 PalForge 界面都会走的挂载路径，面板最多旧 `ms` 毫秒，而当下真正武装的是哪一个，`UI.refreshDriver(ms)` 会告诉你。
