...
Each member takes its own spawn call, and each one's `onSpawned` attaches the mesh to its own
creature as it arrives. The squad does not appear all at once or immediately: every member is a
few seconds behind its own call, so they turn up over the following handful of seconds.
