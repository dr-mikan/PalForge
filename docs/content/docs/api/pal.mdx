...
- Know what `:spawn` can and cannot do in this version

## Make a pal

A pal is one of the game's creatures. Write `Pal{ ... }` with the game's own name for it, and
you get back an object you can look up later, hang behaviour on and act through.
...
`Pal.get` never returns nil. An id nobody defined gets a thin definition — enough to call the
actions on, but with no handlers on it. Only `Pal{ ... }` registers a pal, and only a
...
<Callout type="error" title="Putting a pal in the world is not available in this version">
`:spawn` does not make a creature appear. The game's own spawn call runs without complaining
and nothing arrives — not in that instant, and not a second later either — so `:spawn` reports
what it saw rather than what it asked for, and answers `false`.

What does work is everything about a pal that is **already** in the world.  `onCaptured`,
`onDamaged`, `onDeath`, `onSpawned` and `onTick` all fire for the game's own creatures, and
`:renderOn` still changes what one looks like. Build on the pals the game puts there for you —
the wild ones, the one the player just caught — rather than on creatures you place yourself.

The `toPlayer` route hands the creature to the player's party or box, and nothing in Lua can
look inside either, so it answers `true` for "the call was issued" and cannot promise more than
that. Do not read it as a pal in the box.

The shape below does not change, so code written against it stays correct.
</Callout>
...
Every route goes through `UPalCheatManager`, the game's server-checked admin API. For the two
world routes the result is checked instead of assumed: PalForge counts the creatures in the
world immediately before the call and immediately after it, and answers `true` only when one
that was not there a moment ago is there now. That check is why `:spawn` answers `false` — the
call runs, and no creature is ever there afterwards. A `false` also covers the plain refusals:
an empty id, a coordinate that is not a number, or no cheat manager to reach.

A coordinate is a spawn followed by a move, because the game's own call drops the pal near the
player and ignores the position you asked for. PalForge looks for a creature that was not in
the world before, and teleports that one — retrying every 400 ms, up to six times, since a
creature would take a few frames to appear. With nothing appearing, the pass finds nothing to
move and says so in the log.
...
spawn, and the log records the one time it runs. That is what gives a dedicated server the same
route as a client.

The earliest failure is having no player controller at all: no world loaded, or not connected
yet. Every route then returns `false` without reaching the game, and warns that no cheat
manager was found and none could be built. Calling from `world.ready` avoids that window:
...
Each member takes its own `SpawnMonster` call, and each one's `onSpawned` would attach the mesh
to its own creature. No member appears in this version, so nothing is placed and no `onSpawned`
follows — the same code is what summons the squad once the world route works.

## Limits

<Callout type="warn">

- **Putting a pal in the world does not work.** `:spawn` returns `false` on every route that
  places a creature in the world, and none appears. Everything else on this page is about pals
  the game itself put there, and all of that works.
- **Lua cannot add a brand-new creature.** `Pal{ id = ... }` gives behaviour, visuals and
  ...
  job. An id with no matching `BP_<Id>_C` in the game never gets its handlers called.
...
- **`:spawn` needs a player controller before it can even try.** It needs `PalCheatManager`
  too, but nothing else has to create that first: when the session has none, `core/spawn`
  builds one from the controller's `CheatClass` and attaches it, so a dedicated server takes
  the same route as a client even though CheatManagerEnabler's `ClientRestart` hook never runs
  there. With no `PalPlayerController` to build it on, every route returns `false` without
  reaching the game.
- **A coordinate is a move after the spawn, not a spawn location.** The game's own call ignores
  the position you asked for, so PalForge looks for the creature that just appeared and
  teleports it, retrying every 400 ms up to six times. That pass only ever reports to the log,
  and what it reports today is that there was no new creature to move.
...
- `pal:spawn()` is how a pal would be placed, but it cannot place one in this version: it
  answers `false` and nothing appears. Work with the creatures the game spawns itself.
