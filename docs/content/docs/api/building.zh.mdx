...
**没有记录会因为「不在」而被删掉。** 连续六次**进行了枚举的**扫描都没看到的建筑物，会以 `reason = "missing"` 发出 `building.remove`，但它的记录不是被丢掉，而是被**隔离**：带着 `why = "missing"` 挪进 `orphans`，而下一次再看到那个 Actor 的扫描会把它连同 `state` 一起取回来。那次扫描只枚举内存里的对象，而出货二进制自己的声明表明 Palworld 是按距离生成和销毁地图对象的，所以「这一遍扫描里没有」不能读成「已经没了」。
