...
`:spawn` 在这个版本里什么都不会放下，所以那个 `if` 的内容不会执行，这个台子的充能也就保住了：建筑物、
冷却和存档计数都能用，只有召唤这一步在等 [Pal](/docs/api/pal)。再定义一次 `PalBoxV2`，会替换掉
`native/buildings.lua` 里现成的定义，所以这里把 mesh 重新写了一遍。原版的帕鲁盒子界面照常打开 —— 交互钩子只是看着这次调用，并不会把它吞掉。
