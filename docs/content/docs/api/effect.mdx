...
The names are the game's own spelling, and upper and lower case do not matter. The ones a pack
usually reaches for are `Poison`, `Stun`, `Sleep`, `Burn`, `Freeze`, `Electrical`, `Darkness`,
`Wetness`, `AttackUp` and `DefenseUp`. There are 38 in all, and
`require("palforge.core.status").names()` hands back the whole list, sorted.
...
- `nativeStatus` switches one of the game's own ailments on while your effect runs; what the
  effect *does* beyond that is your code, inside `onApply`, `onTick` and `onExpire`.
