...
<Callout type="warn">
An effect you write does **not** switch on the game's own status icon — the little poison
or burn symbol Palworld shows on a health bar. Those live in the game's own list
(`EPalStatusEffectType`), and there is no confirmed way to turn one on from Lua yet. The
`nativeStatus` field is stored on your definition, and nothing reads it back. So put
whatever the effect is meant to *do* in your handlers: heal, damage, buff or log inside
`onApply`, `onTick` and `onExpire`. The timing around them works today.
</Callout>
...
One target can carry one application of each effect id. The table holding them uses
**weak keys**: an application never keeps a pawn alive, and a target that is cleaned up by
Lua takes its applications with it.

A target counts as alive when it is a plain Lua table, or when it is engine data whose
`IsValid()` still returns true. Once a pawn goes invalid, the next heartbeat ends its
applications with `reason = "target_gone"`.
...
## The three ready-made ailments

`native/effects.lua` ships three definitions, registered for you at load by
`registry.initialize`. Reach them through the module or through `Effect.get` — both give
...
The schedule these three run is real: apply `Poison` and it lives for ten
heartbeat-rounded seconds, firing every second. What is missing is the payload — the
...
<Callout type="warn">
`effects.get` only knows `Poison`, `Burn` and `Freeze`; anything else comes back `nil`.
`Effect.get("Sleep")` still works, but what you get is a fresh empty definition, not one of
the game's ailments. For anything outside those three names, define your own effect.
</Callout>
...
Keep your own table instead, keyed by target, with weak keys, and clear the entry in
`onExpire`:
...
Weak keys matter here for the same reason the runtime uses them: a normal reference from
your table would keep a despawned pawn alive. A world-wide application is keyed on `nil`,
which a Lua table cannot hold, so use a marker object of your own if you need state for
that case.
...
the tick a body by defining your own effect next to it, and apply both, so the pack keeps
working when the game's own ailment gets wired up later.
...
- What the effect does to the character is your code, inside `onApply`, `onTick` and `onExpire`.
- Anything you want to remember per target goes in your own weak-keyed table, cleared in `onExpire`.
