...
<Callout type="info">
效果还可以点亮**游戏自己的状态异常** —— 就是 Palworld 在血条上显示的中毒、燃烧、冰冻那些小符号。
在 `nativeStatus` 里写上一个名字，效果被施加时它就亮起来，效果结束时它就熄灭。除此之外效果实际
要*做*的事，仍然写进处理函数：回复、伤害、增益或者打日志，都放进 `onApply`、`onTick` 和
`onExpire`。名字清单和相关规则见下面的 **游戏自己的状态异常**。
</Callout>
...
<TypeTable
  type={{
    ...
    nativeStatus: { description: '它镜像的那个游戏自带状态异常，比如 "Poison"。效果运行期间它是亮着的。你定义的那一刻就会拿游戏的清单校验', type: 'string' },
...
`nativeStatus` 写的是 Palworld 自己的一个状态异常的名字。声明了它，效果被施加时 PalForge 就会点亮
那个真正的状态异常，效果被移除或到期时再把它熄灭 —— 血条上出现的就是游戏自己会放上去的那个符号，
行为也完全按游戏的规则来。
...
它是**镜像，不是替代**。状态异常按游戏的规则运行，它有多强不由 PalForge 决定。PalForge 决定的是
*什么时候*：把它摘掉的，是你写的 `duration`。

<Callout type="info">
**这件事是观测到能用的**，在一个加载好的存档里，而且值得说清楚「能用」覆盖了什么。路径是
`PalCharacter.StatusComponent`——每个角色都带的一个属性——到
`UPalStatusComponent:AddStatus(EPalStatusID)`，一个整数参数，没有结构体也没有 `FName`。
`status.add` 把 `AttackUp`（`EPalStatusID` 26）加到一个角色身上，游戏通过 `GetExecutionStatus`
把这个状态异常读回来是「有」，`status.remove` 摘掉之后再读回来是「没有」。这些签名来自一份比已安装
二进制早一个补丁的转储，而实机的类三处全都对得上；每次调用现在仍然要过 `core/signature`，实机的类
没有声明就不会发出去。所以 `false` 的意思是*这次调用没有发出去*，绝不是*目标免疫*。
</Callout>

名字用的是游戏自己的拼写。整合包最常用的是 `Poison`、`Stun`、`Sleep`、`Burn`、
`Freeze`、`Electrical`、`Darkness`、`Wetness`、`AttackUp` 和 `DefenseUp`。一共有 38 个，
`require("palforge.core.status").names()` 会返回排好序的完整清单。

### 大小写，以及你此刻在哪一层

`nativeStatus` **不区分大小写**：`"poison"` 和 `"Poison"` 一样能找到 `EPalStatusID` 5，因为
`core/status` 在真表旁边还留了一张全小写的映射。这在这里是安全的，理由却不能推广——这套词汇是固定
的、封闭的，而且整份都随代码发出，所以折叠大小写的代价只是一张表，省下的是记住游戏自己怎么大写。

另一层则**区分大小写**，而且没有任何办法把它变软。指向 DataTable 行或 `object_manager` 注册键的
id——`"pack_Potion"`、`Item.get`、图标行查询——必须一字不差，因为那套名字属于游戏，PalForge 手上没有
可以拿来折叠的对照表。

| 这个名字指的是什么 | 例子 | 大小写 |
| --- | --- | --- |
| 引擎枚举 | `nativeStatus = "poison"`、技能的 `EPalWazaID` | 不区分 |
| DataTable 行或注册 id | `Item.get("Wood")`、`"mypack:Potion"` | 区分 |

状态异常的名字永远不会是行 id，所以 `Effect` 内部不会跨过这条边界——但你的内容包会，就在你把一个
`Item` id 写在 `nativeStatus` 旁边的那一刻。

游戏里没有的名字，会**在你写定义的那一刻**就被拒绝，而不是过后悄悄被忽略，而且消息里会列出你可能
想写的每一个名字：
...
<Callout type="info">
如果状态异常点不亮 —— 目标不是角色，或者游戏不接受这个调用 —— **效果依然会正常施加**，你的处理函数
也照样运行，原因会写进日志。整合包真正依赖的是 PalForge 自己的计时、叠层和处理函数，这些都不依赖
那个图标。
</Callout>
...
一个目标上，每个效果 id 只能带一次施加。存放它们的表，键是目标的 `GetFullName()`，而不是你传进来
的那个句柄。UE4SS 每次查询都会新造一个 userdata 包装，所以指向同一个 pawn（游戏里的角色实体）的两个
引用并不是同一个 Lua 值；用句柄索引的表，会把你的施加记在调用方碰巧拿着的那个包装底下。普通的 Lua
表目标本身身份稳定，就用它自己当键；而一个答不出自己名字的对象则退回用句柄。

目标算活着，有两种情况：它是一张普通的 Lua 表，或者它是引擎数据而 `IsValid()` 仍然返回
true。每次心跳都会检查所有还在跑的施加，所以 pawn 一旦变成无效，下一拍就用
`reason = "target_gone"` 结束它身上的施加，并把空掉的桶丢掉。键变成字符串之后，正是这趟清扫在给
这张表兜底：字符串键不会被回收，没有垃圾回收器可以指望——而且这趟清扫本身是更强的保证，它在 pawn
消失的那一拍就跑，而不是等 Lua 下一次回收。
...
`native/effects.lua` 把游戏声明的每一个状态异常都变成了可以直接施加的效果。没有另一份需要维护的
名单：这份目录就是 `nativeStatus` 接受的那 38 个名字。
...
`effects.get` 返回的句柄带着 `nativeStatus`，但**没有 `duration`** —— 这是它唯一让人意外的地方：
施加之后状态异常就亮着，而且一直亮着。没有任何东西会替你把它摘掉，所以要自己摘。
...
其中三个不一样，值得知道是哪三个：`Poison`、`Burn` 和 `Freeze` 写好了计时，所以它们会自己结束。
...
`effects.get("Burn")` 给你的就是这个带计时的句柄，而不是另造一个没有 `duration` 的。这三个是用自己的
id 注册好的，所以 `Burn` 只有一个，就是带着那五秒的那个。`Effect.get("Burn")` 找到的也是同一份定义。

三个都没有声明处理函数，因为不需要：状态异常本来就按游戏说的那样起作用，而一个空的处理函数只会在
每次 tick 上白花一点力气。想在它之上加自己的东西 —— 伤害、提示、计数 —— 就用同样的 `nativeStatus`
配上 `onTick`，自己声明一个效果，下面的 **持续伤害** 就是这个写法。

<Callout type="info">
`CATALOG` 里的名字并不是每一个都是整合包想要的状态异常。这份清单是游戏自己的，所以它也包含内部的
计时器和记账项 —— `ControlSP`、`DrownCheck`、`UNKOTimer`、`Moratorium`，以及 `PalEnhancement` 系列。
它们全都照原样提供，而不是被悄悄剔掉：剔掉反而会让人以为不支持，其实只是它们比较古怪而已。整合包
最常用的，是上面 **游戏自己的状态异常** 里列出的那十个。
</Callout>
...
弱引用键能防止这张表把一个已经消失的 pawn 一直留在内存里。它给不了你的是一个稳定的键：用 UE4SS
句柄索引的表是按 userdata 身份索引的，而 `onApply` 里的句柄不一定和之后某次 `onTick` 里的句柄是同
一个 Lua 值——`:apply` 每次重新施加时，都会用调用方的句柄刷新这次施加保存的那个。如果你的记账需要
挺过一次重新施加，就照运行时的做法，用名字做键：
...
全局的施加以 `nil` 为键，而 Lua 的表不能用 `nil` 当键，所以那种情况下如果需要状态，请自己用一个
标记对象。
...
`Poison` 已经声明了计时，10 秒、每 1 秒一次，也会点亮游戏自己的中毒，但它没有 `onTick`，所以
不会自己造成伤害。在它旁边定义一个你自己的效果，给 tick 补上内容，然后两个一起施加：游戏负责
显示状态异常，你的效果负责伤害。
...
- `nativeStatus` 会在效果运行期间点亮游戏自己的状态异常——通过 `PalCharacter.StatusComponent`，
  已观测到能用——而且它那 38 个名字不区分大小写，这一点和道具 id 或注册键不同。除此之外效果对角色
  做了什么，是你写在 `onApply`、`onTick` 和 `onExpire` 里的代码。
- 想为每个目标记住的东西，放进你自己的表，在 `onExpire` 里清掉。需要挺过重新施加的话，用
  `uo.key(target)` 做键。
