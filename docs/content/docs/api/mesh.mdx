...
| a texture | **untested** | `ImportFileAsTexture2D` is wired with a signature read off the shipping binary, and no run in this tree has ever called it. A `/Game/...` texture works today |
...
<Callout type="warn">
Nobody has watched a colour actually change. The writes are declared exactly as they are
called, so "the write did not execute" is eliminated, and the names above are real rather than
guessed — but a tint landing on screen is a third observation, and it has not been made. That
is what `pf_hook mesh-color-change` exists to record.
</Callout>
...
- The four painting fields reach every backend, and the parameter names they write were read
  off the running game. Nobody has yet watched a colour change.
