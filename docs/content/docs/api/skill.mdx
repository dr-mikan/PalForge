...
<Callout type="warn">
That list is only a list. PalForge does not look the ids up, does not equip the passives and does
not fire the actives. It stores the list and hands it back through `pal:skillsOf()`. Do the rest
in a handler — the loop below is all it takes.
</Callout>
...
- `pal:skillsOf()` gives you the ids a pal carries; looping over them is your job.
