...
- **The active-move write is measured too, as of 2026-08-02.** `pf_hook pal-skills-equip`
  returned **8 pass / 0 fail** on a real save: `AddEquipWaza` equipped `Human_Punch`
  (`EPalWazaID 1`) and it read back off a live `PalMonsterCharacter`, `:forget` took it off,
  `teachAll(pal)` answered `2, 2`, and `ClearEquipWaza` cleared the loadout with every removed
  move restored and verified afterwards. **The game stayed up.**
- **The crash that used to sit here was the target, not the write.** An earlier run was followed
  1.4 seconds later by Palworld closing, and that run searched `FindAllOf("PalCharacter")` — which
  is too wide. The hierarchy is `APalMonsterCharacter : APalNPC : APalCharacter`, so the wide name
  also matches villagers and merchants, none of which has an equipped move. Ask for
  `PalMonsterCharacter`, as `:skillsOn` and the hook now both do.

<Callout type="warn">
Writing an **active move** into a real character is settled, and it is still worth doing on a save
you do not mind losing the first time — `env.debugHooks["pal-skills-equip"]` keeps the hook's own
write behind a per-experiment opt-in for that reason, not because the outcome is in doubt. Check
the boolean the call returns rather than assuming: `:teach` reports what the game did, and a pack
that reads it is a pack that notices the day the game moves.
</Callout>
...
- `pal:skillsOf()` gives you the ids a pal *declares*. `skill:skillsOn(actor)` reads back the four
  lists a live character really has, `skill:teach(actor)` puts one on — measured for a passive and,
  since 2026-08-02, for an active move as well — and `pal:teachAll(actor)` does the whole declared
  list, which answered `2, 2` on a live pal.
