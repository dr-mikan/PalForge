...
`active` may simply have nothing equipped. `nil` for the whole table means the character could
not be read at all — unknown, never "has none" — so check for `nil` before you index it.

```lua
local carried = Skill.get("FireBlast"):skillsOn(pawn)
if carried then
    print(#carried.active .. " equipped, " .. #carried.equipable .. " available")
end
```

<Callout type="info">
Ask a **pal**, not a villager or a merchant. Those are characters too, and the game's own list
of characters includes them, but they carry no moves — so asking one reports empty lists that
look exactly like a read that failed.
</Callout>
...
<Callout type="warn">
Check that boolean rather than assuming. Reading a loadout is settled; writing to one is not
something to build a pack around yet, and a `false` from `:teach` is the framework telling you
so rather than a bug in your code. Try one in a save you do not mind reloading before you rely
on it.
</Callout>
