...
<Callout type="warn">
Nothing calls `refresh()` for a pack. An element either calls `:refresh()` by hand when its own
state changes, or opts into `:autoRefresh(ms)` — **and `autoRefresh` rides two things, not one.**
An earlier version of this page said polling was the only driver PalForge had; that was overturned
by a measurement on the same day, and the correction is here.

- **The game's own rebuild signal.** `CommonActivatableWidget::ActivateWidget`,
  `PalHUDService::Push` and `PalHUDService::Close` all fire when Palworld builds or tears down a
  screen — 21 candidates armed by path on 2026-08-02, three of them fired. `native/ui/refresh.lua`
  arms those three, lazily and at most once per session, and a refresh lands within one heartbeat
  of a rebuild.
- **The heartbeat, underneath, as the floor.** It is not removable and it is not a fallback. 18 of
  the 21 candidates stayed silent in that run, and that is an absence of *input* — the operator
  opened two screens, not eighteen — so a rebuild those three do not cover is exactly what the
  floor is for. The signal is also armed only after `world.ready`, because UE4SS cannot unregister
  a hook and `ActivateWidget` fires for every activatable during a world-load storm; a title-screen
  element therefore runs on the poll alone.

So a panel shows **stale content for at most `ms` milliseconds whatever happens, and for about one
heartbeat after a screen rebuild**. Shortening `ms` is still the only lever on the first number. A
beat over an idle panel is cheap on purpose — bindings compare and write back only what changed —
so a fast `ms` costs comparisons, not native calls, and riding the signal does not slow the floor
down.
</Callout>
...
Option two — subscribe. Both subscriptions ride `event.every`, which counts whole heartbeats of
`event.TICK_MS` (500 ms), so the floor's real period rounds up to a multiple of 500; the rebuild
signal, when it is armed, adds a refresh on top of that beat rather than replacing it.
...
<Callout type="info">
**Ask what is actually driving a refresh rather than trusting this page.** `UI.refreshDriver(ms)`
reads the live arming and hands the answer back as data:

```lua
local d = UI.refreshDriver(250)

d.kind     --> "event+poll" when the rebuild signal is armed, "poll" when it is not
d.event    --> the UFunction path(s) armed right now, or nil
d.state    --> "armed" | "waiting" | "refused" | "unavailable" — why kind is what it is
d.staleMs  --> how stale a panel may be when nothing rebuilt a screen (the floor's number)
d.eventStaleMs --> how long after a rebuild the refresh lands: one heartbeat, or nil
d.why      --> one sentence naming the measurement, ready for a log line
```

`kind == "poll"` is never a failure state — it is what shipped before the signal existed, and it
is the honest answer at the title screen, in a session where arming was refused, and in a headless
test where there is no `RegisterHook` at all. Branch on `kind` only to *say* how stale a readout
may be, never to decide whether refreshing works: the floor is under every answer.

The measurement itself is `pf_hook ui-update-event`, and it can be re-run: the 18 candidates that
stayed silent — `PalHUDService:ShowCommonUI` for popups, `RemoveHUD` for a return to title, the
world-HUD pair, the CommonUI container's add/remove — were never given an action that would
exercise them, so they stay named in the hook rather than written off.
</Callout>
...
| `:autoRefresh(ms)` | `boolean` | drive `refresh()` off Palworld's rebuild signal AND the heartbeat as the floor; `ms` defaults to 500 |
| `:autoMount(root, ms)` | `boolean` | the whole lifecycle on the same driver: retry `mount` while down, refresh while up; `ms` defaults to 2000 |
...
| `UI.refreshDriver(ms)` | `table` | what actually refreshes a panel right now: `kind`, `event`, `state`, `staleMs`, `why` |
...
- Refreshing rides two drivers: Palworld's own rebuild signal, and the heartbeat as the floor
  underneath it. `:autoMount(root, ms)` is the mount path every live PalForge UI takes, a panel is
  stale by at most `ms`, and `UI.refreshDriver(ms)` says which driver is actually armed.
