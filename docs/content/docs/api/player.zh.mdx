...
<Callout type="warn">
按坐标生成不是立刻完成的。`:spawn{ at = ... }` 调用的是 `PalCheatManager:SpawnMonster`，
它把帕鲁放在玩家旁边，随后 PalForge 再把那一个 actor 挪到你给的坐标上，每 400 ms 重试一次、
最多六次，因为 actor 是在调用之后几帧才真正出现的。所以你会看到它先出现在你身边，然后再移动
过去。如果没装 `CheatManagerEnabler` 模组，就根本没有 cheat manager，`:spawn` 返回 `false`。
</Callout>
