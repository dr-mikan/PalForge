...
<Callout type="warn">
このバージョンでは `Pal.Handle:spawn` はワールドにクリーチャーを配置しないので、ここで求めた座標が
何かをどこかに出すことはありません。[Pal](/docs/api/pal) を参照してください。座標そのものは本物で、
このページの他の内容はすべて動きます。自前の距離計算、建築物の位置、自分で配置するものなどに
使ってください。

経路としては、スポーンのあとに移動が続く形です。ゲーム自身の呼び出しはプレイヤーのすぐそばにパルを
出し、PalForge はそのアクターだけを 400 ms 間隔で最大 6 回まで再試行しながら指定座標へ移動させます
（クリーチャーが現れるまで数フレームかかるためです）。また `CheatManagerEnabler` mod が必要です。
無い場合は管理用オブジェクトがそもそも存在せず、`:spawn` はゲームに届く前に `false` を返します。
</Callout>
