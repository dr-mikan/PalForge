...
帕鲁是一种可以生成出来的生物。用游戏里那只生物本来的名字写一句 `Pal{ ... }`，你就会拿到一个
对象，可以用它生成帕鲁、之后再取回它、给它挂上行为。
...
`skills` 只是一串 id，没有别的含义。PalForge 不会给帕鲁装备它们，游戏里也没有任何东西会触发
它们。用 `:skillsOf()` 把这串 id 读回来，再通过 [`Skill`](/docs/api/skill) 自己去释放每个技能。
...
**这份定义自己的句柄**——就是 `Pal{ ... }` 返回的那个对象，所以 `:spawn`、`:renderOn` 和各个查询
...
每条路都走 `UPalCheatManager`，也就是游戏那套由服务器校验的管理 API。`true` 表示这次生成调用被
**接受**了，不表示已经有一只帕鲁站在某个地方：生物要过几帧才出现，而坐标那条路随后还要在一轮
远晚于 `:spawn` 返回才结束的处理里把它挪过去。那一轮只向日志汇报，不会回到你的调用处。`false`
则是确定的——什么都没试过，因为 id 是空的、坐标不是数字，或者根本够不到作弊管理器。

走坐标这条路时，游戏会把帕鲁扔在玩家附近，忽略你要求的位置。所以 PalForge 先给现有的帕鲁拍一张
快照，再把新出现的那只里离玩家最近的一只挪过去——每 400 毫秒重试一次，最多六次，因为生物要过
几帧才会出现。
...
剩下会失败的情况，是根本没有玩家控制器：世界还没加载，或者还没连上。这时每条路都返回 `false`，
并警告说找不到作弊管理器，也造不出来。从 `world.ready` 里生成可以避开这段时间：
...
`Item.get(...):give(n)` 走游戏自己的 `AddItem_ServerInternal` 路径，把东西加进本地玩家的背包。
PalForge 没有把东西丢到地上的调用，所以这是玩家直接收到的奖励，不是走过去捡的一袋东西。道具还
能做什么，见 [Item](/docs/api/item)。
...
每个成员都通过各自的 `SpawnMonster` 调用生成，各自的 `onSpawned` 把网格装到各自的生物上。
...
- **处理函数出错，这个处理函数就到此为止。** PalForge 在 `pcall` 里调用你的钩子，并把失败连同
  ...
  游戏都不会察觉。
- **`:spawn` 需要一个玩家控制器。** 它也需要 `PalCheatManager`，但不必先由别的东西创建：会话里
  一个都没有时，`core/spawn` 会用控制器的 `CheatClass` 造一个并挂上去，这就是 `:spawn` 在专用
  服务器上也能用的原因——那里 CheatManagerEnabler 的 `ClientRestart` 钩子从不运行。如果连
  `PalPlayerController` 都没有，没有东西可以拿来造，每条路都返回 `false` 并警告。
- **坐标是生成之后的移动，不是生成位置。** 游戏自己的调用会忽略你要求的位置，所以 PalForge 在
  生成之后把新出现的生物传送过去，每 400 毫秒重试一次，最多六次。`:spawn` 返回的 `true` 只针对
  生成调用本身；移动的结果只会写进日志——没有新生物出现时警告一次，找到了但每一次移动调用都失败、
  生物留在原地时再警告一次。没有 `LoopAsync` 和 `ExecuteInGameThread` 的环境里根本没有这段延后的
  处理，帕鲁就待在玩家旁边。
...
- 帕鲁身上发生的事写在 `events` 下面：`onSpawned`、`onDamaged`、`onDeath`、`onCaptured` 和
  `onTick`。这五个都是可用的。
- `pal:spawn()` 把帕鲁放进世界。想指定地点就传坐标，想直接送给玩家就传
  `{ toPlayer = true, num = 3 }`。
