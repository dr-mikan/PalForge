...
- 弄清这个版本的 `:spawn` 能做什么、不能做什么

## 创建帕鲁

帕鲁就是游戏里的生物。用游戏里那只生物本来的名字写一句 `Pal{ ... }`，你就会拿到一个对象，
之后可以再取回它、给它挂上行为、通过它做各种操作。
...
`Pal.get` 永远不会返回 nil。没有人定义过的 id 会拿到一份很薄的定义，够用来调用那些动作，但上面
...
<Callout type="error" title="这个版本没法把帕鲁放进世界">
`:spawn` 不会让生物出现。游戏自己那个生成调用一声不吭地跑完了，可什么都没来 —— 当下没有，一秒
之后也没有 —— 所以 `:spawn` 报告的是它真正看见的东西，而不是它请求的东西，返回 `false`。

能用的是围绕**已经**在世界里的帕鲁的一切。`onCaptured`、`onDamaged`、`onDeath`、`onSpawned` 和
`onTick` 都会对游戏自己的生物触发，`:renderOn` 也照样能改变它的外观。请围绕游戏替你放进世界的
帕鲁来做内容 —— 野生的那些、玩家刚捕到的那只 —— 而不是围绕你自己放置的生物。

`toPlayer` 那条路是把生物交给玩家的队伍或仓库，而这两个地方 Lua 都看不进去，所以它返回的 `true`
只表示“调用发出去了”，不能保证更多。别把它当成仓库里真的多了一只帕鲁。

下面这个写法不会变，照着它写的代码以后依然正确。
</Callout>
...
每条路都走 `UPalCheatManager`，也就是游戏那套由服务器校验的管理 API。对于放进世界的那两条路，
结果是被核对过的，不是假定的：PalForge 在调用之前和调用之后各数一次世界里的生物，只有当一只刚才
还不在的生物现在在了，才返回 `true`。`:spawn` 之所以返回 `false`，正是因为这道核对 —— 调用跑完了，
而之后从来没有生物在那儿。`false` 也涵盖那些直白的拒绝：id 是空的、坐标不是数字，或者根本够不到
作弊管理器。

坐标是“先生成、再搬过去”。游戏自己那个调用会把帕鲁扔在玩家附近，忽略你要求的位置，所以 PalForge
会去找刚才还不在世界里的那只生物，把它传送过去 —— 每 400 毫秒重试一次，最多六次，因为生物本该过
几帧才出现。今天什么都不会出现，所以这一轮找不到可搬的对象，并把这件事写进日志。

#### 作弊管理器

`SpawnMonster` 和 `SpawnMonsterForPlayer` 是 `UPalCheatManager` 上的方法。在客户端上，
CheatManagerEnabler 模组会在 `PlayerController:ClientRestart` 里创建这个对象；在专用服务器上那个
钩子从不触发，也没有别的东西会去创建它。会话里一个都没有时，`core/spawn` 会自己造一个：它用
`StaticConstructObject` 构造 `PalPlayerController` 自己的 `CheatClass`——那个类为空时依次回退到
`/Script/Pal.PalCheatManager` 和 `/Script/Engine.CheatManager`——再把结果挂到控制器上。这个对象会
一直留着，所以这件事每个会话只发生一次，而不是每次生成都发生一次，唯一的那次会记进日志。这就是
专用服务器能和客户端走同一条路的原因。

最早的失败，是根本没有玩家控制器：世界还没加载，或者还没连上。这时每条路都在够到游戏之前就返回
`false`，并警告说找不到作弊管理器，也造不出来。从 `world.ready` 里调用可以避开这段时间：
...
每个成员各用一次自己的 `SpawnMonster` 调用，各自的 `onSpawned` 再把网格装到各自的生物上 —— 这是它
应有的形状。这个版本里一个成员都不会出现，所以什么都没被放置，`onSpawned` 也不会跟上。等放进世界
那条路能用了，同样这段代码就会把分队召出来。

## 限制

<Callout type="warn">

- **没法把帕鲁放进世界。** 凡是把生物放进世界的那几条路，`:spawn` 都返回 `false`，而且什么都不会
  出现。本页其余内容讲的都是游戏自己放进世界的帕鲁，那些全都能用。
...
- **处理函数出错，这个处理函数就到此为止。** PalForge 在 `pcall` 里调用你的钩子，并把失败连同
  ...
  游戏都不会察觉。
- **`:spawn` 连尝试都需要先有一个玩家控制器。** 它也需要 `PalCheatManager`，但不必先由别的东西
  创建：会话里一个都没有时，`core/spawn` 会用控制器的 `CheatClass` 造一个并挂上去，所以专用服务器
  能和客户端走同一条路——那里 CheatManagerEnabler 的 `ClientRestart` 钩子从不运行。如果连
  `PalPlayerController` 都没有，没有东西可以拿来造，每条路都在够到游戏之前就返回 `false`。
- **坐标是生成之后的移动，不是生成位置。** 游戏自己的调用会忽略你要求的位置，所以 PalForge 会去找
  刚出现的那只生物并把它传送过去，每 400 毫秒重试一次，最多六次。这一轮的结果只会写进日志，而今天
  写进去的内容是：没有新生物可搬。
...
- 帕鲁身上发生的事写在 `events` 下面：`onSpawned`、`onDamaged`、`onDeath`、`onCaptured` 和
  `onTick`。这五个都是可用的。
- `pal:spawn()` 本来是放置帕鲁的调用，但这个版本放不进去：它返回 `false`，什么都不会出现。请围绕
  游戏自己放出来的生物来做内容。
