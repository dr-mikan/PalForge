...
silence. It returns `true` when the call was issued, which is as far as the game lets Lua see —
nobody has yet confirmed by ear what a given number sounds like, so treat the number as
something to tune rather than a promise.
...
Three things here do less than you would expect: one is unfinished, and two reach further than
...
`:setVolume` shares `:stop`'s actor-wide scope, covered above: it scales everything that actor
is emitting rather than the one sound you called it on. A per-sound volume slider still has to
be separate actors, or separate events.
