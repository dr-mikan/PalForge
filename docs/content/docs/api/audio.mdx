...
<Callout type="error" title="Setting a sound's volume is not available in this version">
`:setVolume` changes nothing and always returns `false`. There is no per-sound volume dial for
it to turn: Wwise, the sound system Palworld uses, lets a mod set named parameters, and this
build of the game declares exactly three of them — none of which is a volume — and no mixing
buses at all.

**Choosing the event is the volume control.** The catalog holds loud sounds and quiet ones for
the same kind of moment, so pick the one that already sits at the level you want, and leave the
overall loudness to the player's own audio settings.
</Callout>
...
`:setVolume` is the third gap, covered above: it returns `false` and changes nothing, and there
is no volume to set on this build, so a volume slider has nothing to move. Pick a quieter event
instead.
...
- `:stop(actor)` silences everything on that actor, and `:setVolume` does nothing — choose a
  quieter event instead.
