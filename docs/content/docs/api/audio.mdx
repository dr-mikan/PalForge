...
was playing, and it has never been playable on any build this tree has measured. Both halves of
the engine are closed to it — `USoundWave` declares no importer and `USoundWaveProcedural`
declares nothing at all, so nothing survives shipping to read a `.wav`, and the Wwise half
(`UWwiseExternalSourceStatics::SetExternalSourceMediaBy*`) rebinds media the Wwise cook already
staged, which is not a file on a pack's disk. Pick the closest sound from the catalog instead.
</Callout>
