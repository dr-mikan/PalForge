...
Defining `PalBoxV2` again replaces the ready-made definition from `native/buildings.lua`, so
the mesh is restated here. The vanilla pal box UI still opens — the interact hook watches the
