...
`:spawn` places no creature in this version, so the `if` never runs its body and the post keeps
its charge — the structure, the cooldown and the saved counter all work, and the summon is the
one step waiting on [Pal](/docs/api/pal). Defining `PalBoxV2` again replaces the ready-made
definition from `native/buildings.lua`, so the mesh is restated here. The vanilla pal box UI still opens — the interact hook watches the
