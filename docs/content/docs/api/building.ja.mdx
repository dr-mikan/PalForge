...
return Building{
    ...
    events = {
        ...
        onRightClick = function(self, ctx)
            ...
            if Pal.get("ChickenPal"):spawn(at) then   -- false in this version: no pal appears
...
`:spawn` はこのバージョンでは何も配置しないので、`if` の中身は実行されず、この台はチャージを保った
ままになります。建築物・クールダウン・保存されるカウンターはすべて動き、召喚だけが
[Pal](/docs/api/pal) 待ちです。`PalBoxV2` を定義し直すと `native/buildings.lua` の用意済み定義を置き換えるため、mesh をここで
