...
`saveId` は `core/spatial` が決めます。生きている `PalGameInstance` から選択中のセーブを読み、
...
得られた名前の英数字とアンダースコア以外を `_` に置き換え、先頭に `w_` を付けたものが saveId で、
何も読めなければ単に `world` になります。

<Callout type="info">
`WorldGuid`、`WorldSaveName`、`SaveName` はこのクラスにありません。`PalGameInstance` の
111 プロパティの完全な一覧にどれも載っていないので、この 3 つの名前に基づくプローブは `world`
フォールバックにしか行き着けませんでした。それがすべてのセーブで永続化ファイルが 1 つに
なっていた理由であり、実セッションが残した唯一の成果物が `entities_world.json` という名前である
理由でもあります。表示名よりディレクトリ名を優先するのは、2 つのセーブがセーブフォルダを共有する
ことはできない一方、プレイヤーが入力したラベルは重複し得るからです。
</Callout>
...
キーは解決済みの **build** id とセルなので、所有者をまったく名乗りません。そこで所有者はレコードの
中に乗ります。`v` がレコードの形、`def` がそれを主張した定義 id、`pack` はレジストリが帰属を
言えるときの所有パック id です。古いレコードは、スキャンが最初に結びつけた時点でその場で
アップグレードされます。すべてのパックが 1 つのファイルを共有する構成で取れる移行経路はそれだけ
です。
...
そのセッションで**どの定義も主張しなかった** build id のレコードは削除されません。隔離されます。
中身をそのままファイルの `orphans` セクションへ移し、件数と所属していたパックをログに出し、その
build id をどれかの定義が再び主張した瞬間に戻します。このパスはワールドが開いてからおよそ 30 秒
待ってから走るので、`world.ready` や遅延で building を定義するパックにも間に合います。隔離が保守的
なのは意図的です。プレイヤーが一晩だけ無効にしたパックのために、設置物の状態を失わせるわけには
いきません。ランタイムでレコードを本当に破棄するのは隔離上限の 4096 件だけで、それを超えると古い
ものから順に落とし、何件をなぜ落としたかをログに書きます。
...
actor 索引、永続化キャッシュを `_G` 上の 1 つのテーブルに置いており、`core/reload` はワイプの際に
`object_manager` を残します。そのため F9 を押しても `:instances()` は答え続け、すでに持っていた
...
- 設置物は丸めたワールド座標で見つけ直されるので、`gridCm` が「どれだけ近くに並べられるか」を
  決めます。そのキーが指すレコードには、所有する定義とパックが書かれています。
