...
| `:take(count)` | boolean | `count` 個（既定 1）をプレイヤーの足元に落として所持品から取り除きます。`count` は絶対値として扱われます。所持数が減ったのを観測できたときだけ `true` です。 |
...
`:give` はゲーム自身の管理用オブジェクトを通してプレイヤーの所持品にアイテムを入れ、そのうえで本当に届いたかを確かめます。呼び出しの前と後に所持数を読み、`true` は所持数が**増えたのを観測できた**という意味です。
...
<Callout type="warn">
`:give` と `:take` はゲームの管理用オブジェクトを通ります。これを有効にするのはコンパニオン Mod の `CheatManagerEnablerMod` です。入っていないと呼ぶ相手が存在しないので、どちらもゲームに届く前に断られ、`false` を返します。`mods.txt` に書く行は [はじめかた](/docs/getting-started) にあります。
</Callout>

戻り値は呼び出しの成否ではなく実測の結果です。`false` は「届いたものが観測できなかった」という意味で、原因はいくつかあります。管理用オブジェクトが無い、ゲームが知らないアイテム ID だった、所持品に空きが無かった、などです。結果はアイテム名・個数・所持数の変化とともに毎回ログに残るので、真偽値では区別できないケースもログで切り分けられます。
...
名前空間付きの ID は先に解決されるため、`Item.get("example:Potion"):give(1)` はゲームに `example_Potion` を渡します。対応するデータ行が存在するかどうかを PalForge は確認しません。ゲームが知らない ID もそのまま受け付けられ、何も増えず、`false` として返ってきます。

<Callout type="info">
自分で呼んだ `:give` が自分の `onObtain` に届くことを当てにしないでください。`onObtain` が追っているのはゲーム自身の入手報告で、`onObtain` ハンドラの中では自分自身を呼び直さないようガードも入ります。何を配ったかは、配ったその場でログに書いてください。
</Callout>

### take

`:take` は同じ実測を逆向きに行います。前後で所持数を読み、`true` は所持数が**減ったのを観測できた**という意味です。

<Callout type="warn" title="take はアイテムをプレイヤーの足元に落とします">
このバージョンにはアイテムを消滅させる呼び出しがありません。そこで `:take` は所持品から取り出す呼び出しを使います。アイテムはプレイヤーが立っている場所の地面に落ちます。所持品が本当に失ったこと自体は `:take` の意味どおりで、所持数の確認がそれを裏づけますが、プレイヤーのすぐ横に山ができ、誰でもその上を歩いて拾い直せます。

この山を前提に設計してください。`:take` は「渡してもらう」には向き、「消費させる」には向きません。対価として使うのは、拾い直されても設計が壊れない場面だけにしてください。
</Callout>
...
プレイヤーの所持数より多く要求してもエラーにはなりません。要求は所持している分に丸められます。1 つも持っていないときは呼び出し自体を行いません。どちらもログに残るので、真偽値では区別できないケースもログで切り分けられます。
...
<Callout type="warn">
丸められた除去でも `true` になります。上の 2 行目は 5 個の要求のうち 2 個を落として `true` を返します。`true` の意味は「所持数が減ったのを観測できた」であって「ちょうど `count` 個減った」ではありません。個数を正確に扱いたいときは、自分で前後に `:count()` を読んでください。
</Callout>
...
`count` は所持品まわりで唯一動く操作です。その ID をローカルプレイヤーがいくつ持っているかをゲームに尋ね、ただの数値を返します。`nil` は「まったく読めなかった」（ワールド未読み込み、プレイヤーが見つからない）という意味で、「0 個」ではありません。比較する前に `nil` かどうかを確かめてください。
...
先に取り、成功したときだけ渡す。この順序が、台がただ働きで払い出すのを防ぎます。`:take` は何も出ていったのを観測できなければ `false` を返すからです。ただし約束しないことが 2 つあります。木材は消えません。プレイヤーのすぐ横の地面にあり、そのまま拾い直せるので、この台は本当の対価ではなく手間の交換です。そして `true` は 5 個すべてが出ていったことを意味しません。要求は所持している分に丸められるので、正確な取引には自分で `:count()` を確かめる処理が必要です。
...
**取り出したアイテムはプレイヤーの足元に落ちます。** このバージョンにはアイテムを消滅させる手段が無いため、`:take` は所持品から世界へ移します。除去そのものは本物で実測されますが、床に落ちた山も本物です。
...
**`give`・`take`・`count` はローカルプレイヤー専用です。** `PalPlayerCharacter` を探してその所持品に作用するので、共有サーバーでは最初に見つかったプレイヤーポーンについて答えます。`count` の `nil` は読み取りに失敗したという意味で、鞄が空という意味ではありません。

**`give` と `take` には管理用オブジェクトが必要です。** どちらも `CheatManagerEnablerMod` が有効にするオブジェクトを通ります。このコンパニオン Mod が無いと、ゲームに届く前に断られて `false` になります。`count` には不要です。

**`:give` が自分の `onObtain` に返ってくることを当てにしないでください。** 自分のコードが何を配ったかは、イベントが回ってくるのを待たずに、呼んだその場でログに書いてください。
...
- `Item.get("Wood")` はどのアイテムでもハンドルを返します。`:count()` は今いくつ持っているかを読み、`:give` は追加し、`:take` は取り除きます。3 つとも「試みたこと」ではなく「実測できたこと」を返します。`:take` が取り除いた分は地面に落ちます。
