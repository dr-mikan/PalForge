...
消費はプレイヤーが持っている武器を経由して行われます。何も装備していないプレイヤーには経由するものが無いので、`:take` は `false` を返し、その旨をログに書きます。設計時に考慮すべきはこの一点です。建築物が対価を取る場合、素手で立っているプレイヤーは支払えません。
</Callout>
...
`iconOf` は `/Game/Pal/DataTable/Item/DT_ItemIconDataTable` から ID を引き、行のテクスチャを読みます。バニラの ID なら、ゲーム本体がそのアイテムに使っている絵がそのまま返ります。各段階は fail-soft で、テーブル・行・カラムのいずれが欠けても宣言済みの `icon` を返します（設定していなければ `nil`）。自作の名前空間付き ID には行が無いので、常にフォールバックします。
...
**取り出しにはプレイヤーが何かを装備している必要があります。** 消費はプレイヤーが持っている武器を経由するので、何も持っていないプレイヤーには経路が無く、`:take` は `false` を返します。アイテム自体は落ちるのではなく消費されます。
