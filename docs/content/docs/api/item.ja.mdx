...
<TypeTable
  type={{
    ...
    restores: {
      description: 'このアイテムを使ったときに回復するもの。ゲームが元から行うことに「加えて」適用されます: { satiety = 20, hpRate = 0.25 }。この spec でキャラクターに書き込む唯一のフィールドです。hp（絶対量）は定義時に拒否されます。',
      type: 'table',
    },
    events: {
...
`recipeOf()` は、宣言されたレシピがあればそれを返し、無ければ**ゲーム自身の行**を読みます。`DT_ItemRecipeDataTable_Common` は `/Game/Pal/DataTable/Item/` に 1414 行で読み込まれており、キーはアイテム id、列は `Product_Count`、`Material1_Id` から `Material5_Id`、`WorkAmount` ほか 9 つです。読み方は `FindRow` 1 回と、返ってきた行構造体から列名で直接引くだけ。2026-08-02 にロード済みのセーブで `pf_hook item-datatable-row-read` が実測しました。列ごとに読む予備経路がコードに無いのはそのためです。**返る形は `api/item.lua` の関数のすぐ横に書いてあります**。下の表は同じことを 1 行で述べています。

宣言したレシピが勝つのは意図的で、このハンドルで宣言が実データに勝つ唯一の場所です。`icon` は「アイコン行が無い id のための予備」と自分で名乗っているのでゲーム側が勝ちますが、レシピは自分のアイテムについて自分が述べた内容です。バニラ id に自前のクラフトを記述したパックは、自分の数値を受け取らなければなりません。すべてフェイルソフトで、ワールドが無い・テーブルが無い・行が無い・行が何も答えない、のいずれも例外ではなく `nil` になります。

`recipeOf()` は経路として実測されただけでなく、ゲームから答えを返すところが観測されています。2026-08-02 のセーブ上で `Arrow -> Arrow x10, work = 1000.0, from { Stone x2, Wood x2 }` と報告しました。

### 食べ物と薬: restores の形

`restores` は、この spec で**キャラクターに書き込む**唯一のフィールドです。これを宣言すると、そのアイテムを使ったときに使った本人の満腹度や体力が回復します。その id に対してゲームが元から行うことに、上乗せする形です。

<TypeTable
  type={{
    satiety: {
      description: '使用時に加算される満腹度のポイント。バーの範囲に丸められます（実測ビルドでは幅 100.0）。ゲーム自身の効果の代わりではなく、それに加算されます。',
      type: 'number',
    },
    hpRate: {
      description: '最大 HP に対する「割合」。0.25 なら 4 分の 1 です。負の値にすると回復ではなく減少し、そちらも実測済みです。',
      type: 'number',
    },
    hp: {
      description: '常に定義時に拒否されます。HP の絶対量は FFixedPoint64 を取り、これは UE4SS が Lua から渡せない構造体です。代わりに hpRate を宣言してください。',
      type: 'number',
    },
  }}
/>
...
これは 2026-08-02 に `pf_hook item-satiety-write` が生きたキャラクター上で実測しました。満腹度は `31.648 -> 21.648` と動いて元に戻され、このビルドの `SetFullStomach` が取る引数は**1 つ**で、`AddHPByRate` は届きます。同じ日の夜には、ゲーム内での動作も確認されました——`item: satiety 68.247 of 100.0, restored -5 and put back`。

<Callout type="warn">
`restores = { hp = 50 }` — HP の**絶対量** — は定義時に例外を投げ、その理由も名指しします。この呼び出しは `FFixedPoint64` を取り、これは UE4SS が Lua から渡せない構造体です。そこで不正な引数を渡すと UE4SS 自身のマーシャリングの内側で落ち、`pcall` からは見えません。代わりに `hpRate` を宣言してください。

`restores` はゲーム自身の効果に**加算される**ものであって、置き換えるものではありません。`Berries` に `restores = { satiety = 20 }` と書くのは「20 の代わりに」ではなく「さらに 20」という意味で、バニラの消耗品はゲームが回復するぶんを引き続き回復します。空の `restores = {}` も拒否されます。どの数値も名指ししない restore は、`item.use` を購読して何もしないだけになるからです。
</Callout>

`Handle:restoreOn(actor)` は、誰かがアイテムを使うのを待たずに、同じ宣言テーブルをいますぐキャラクターへ書き込みます。腹を満たすたき火、クエストの報酬、時間で効くエフェクトなどのためのものです。判定は呼び出しの成否ではなく読み戻しです。宣言したすべての数値が動いたのを確認できたときだけ `true` で、それ以外は `false` と、状態を名指しする英語の理由が返ります（生きたキャラクターがいない、満腹度がすでに満タン、ゲームは呼び出しを受け付けたが何も動かなかった）。
...
| `:recipeOf()` | table \| nil | 自分が宣言したレシピ。無ければ、この id に対するゲーム自身の行をその場で読みます。どちらも無ければ `nil`。 |
| `:restores()` | table \| nil | このアイテムが回復すると宣言しているもの——`{ satiety = 20, hpRate = 0.25 }`——宣言が無ければ `nil`。ゲーム自身の効果はここからは読めず、こちらのものとして報告されることもありません。 |
| `:restoreOn(actor)` | boolean, string \| nil | 宣言された `restores` を、生きたキャラクターへいますぐ書き込みます。宣言したすべての数値が動いたのを確認できたときだけ `true`、それ以外は `false` と理由。 |
...
**レシピの読み取りは動いているゲームで行われ、答えが返ってきました。** 2026-08-02 のセーブ上で `Arrow` が `Arrow x10, work = 1000.0, from { Stone x2, Wood x2 }` として返ってきました。したがって、バニラ id が `nil` を返したときは、そのセッションでレシピ DataTable が読み込まれていたかどうかの問題であって、経路の問題ではありません。ログに `[PalForge.recipes]` が 1 度だけそれを述べます。
...
**`category`、`maxStack`、`recipe` は PalForge 側のメモです。** ハンドル経由と自分のコードから読み戻せるだけで、ゲーム自身のアイテムテーブルに書き込まれることはありません。`maxStack = 9999` と書いても、ゲーム側のスタックの仕方は変わりません。例外は 2 つあり、向きは逆です。`icon` は先に実物のテーブルを読み、宣言したパスはその予備になります。そして `restores` はそもそもメモではありません。ここでキャラクターに書き込む唯一のフィールドです。
...
- `category` と `maxStack` は自分のコード向けのメモで、ゲームは読みません。宣言した `recipe` もメモですが、宣言しなかった場合は `recipeOf` がゲーム自身の行まで読みに行きます。`icon` は、実際にゲームのアイコン行へ届く検索の予備です。
