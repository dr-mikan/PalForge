...
| 纹理 | **可以** | `ImportFileAsTexture2D` 在 2026-08-02 的一次加载好的存档里真的被调用过，返回了一个真正的 `Texture2D`；同一路径的第二次调用由缓存回答。每条路径每个会话会分配一个导入的纹理，并且不会被销毁 —— 缓存正是用来把这个量兜住的。`/Game/...` 的纹理同样能用 |
...
<Callout type="info">
**现在已经有人看着颜色变了。** `pf_hook mesh-color-change` 在 2026-08-02 跑过，有一位操作者盯着
屏幕：空中的一个箱子依次变成**红 → 绿 → 蓝 → 消失**。那正是这条注记一直在等的第三个观测 ——
这些写入本来就是完全照着声明来调用的，上面那些参数名也本来就是从运行中的游戏里读出来的，而不是
猜的。往材质没有的名字上写仍然是悄无声息的空操作，所以要弄清某个 actor 认哪些名字，办法依旧是
`describeMaterials`。
</Callout>
...
- 在帕鲁或建筑物里写 `mesh = ...` 交出去，那边会替你挂上；或者用 `meshHandle:attachTo(actor)`
  自己把它挂到 actor 上。
- 那四个上色字段会送到每一个后端，它们写入的参数名是从运行中的游戏里读出来的，而且已经有人
  看着颜色变过：红 → 绿 → 蓝。
