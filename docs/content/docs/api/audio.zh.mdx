...
<Callout type="error" title="这个版本没法调节单个声音的音量">
`:setVolume` 什么都不会改变，永远返回 `false`。因为根本没有“单个声音的音量旋钮”可以拧：Palworld
用的声音系统 Wwise 允许模组设置具名参数，而这个版本的游戏总共只声明了三个，没有一个是音量，
也完全没有混音总线。

**挑事件就是你的音量控制。** 同一类场合，目录里既有响的声音也有轻的声音，直接挑一个音量本来就
合适的；整体音量交给玩家自己的音频设置。
</Callout>
...
第三处欠缺是上面说过的 `:setVolume`：它返回 `false`，什么都不改，而这个版本上根本没有可设置的
音量，所以音量滑块没有能拖动的东西。改成挑一个更安静的事件。
...
- `:stop(actor)` 会停掉那个 actor 上的一切；`:setVolume` 什么都不做，改成挑一个更安静的事件。
