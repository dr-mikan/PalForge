...
游戏让 Lua 看到的也就到此为止 —— 还没有人用耳朵确认过某个数值听起来是什么样，所以把这个数字当成
可以调的旋钮，而不是一个承诺。
...
这里有三样东西做的事比你预期的少：一样没做完，两样管到的范围比你要求的大。三样都不会报错，
...
第三样是上面说过的 `:setVolume`，它和 `:stop` 的作用范围一样：调整的是那个 actor 正在发出的一切，
而不是你调用它的那一个声音。真要做按声音的音量滑块，还是得靠分开的 actor 或分开的事件。
