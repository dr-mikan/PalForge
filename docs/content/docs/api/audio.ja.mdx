...
<Callout type="warn">
`:setVolume` は**未実装**です。何もしなかったと呼び出し側が判別できるよう `false` を返します。
現時点の PalForge にサウンド単位の音量調整はありません。より静かなイベントを選ぶか、音量は
プレイヤー自身の設定に任せてください。経路自体は製品版のゲームに存在します。
`UPalSoundUtility::SetRTPCValueByActor`、`UAkGameplayStatics::SetRTPCValue`、そしてアクターの
エミッターに対する `UAkComponent::SetOutputBusVolume` です。ただしどれもサウンド単位ではなく、
引数リストもまだ判明しておらず、Palworld が宣言している RTPC は `Field_Time` と `Sliding_Speed`
だけで、音量用の名前は分かっていません。実装するとシグネチャは `setVolume(volume, actor)` に
変わるか、バス単位の呼び出しに移る可能性が高いです。
</Callout>
...
3 つ目の欠落は上で触れた `:setVolume` です。`false` を返して何もしないので、まだこれを土台に
音量スライダーを作らないでください。
...
- `:play()` の `true` は呼び出しが出たという意味で、聞こえた保証ではありません。無音のときは
  `:source().path` を確認します。
- `:stop(actor)` はそのアクター上のすべてを止め、`:setVolume` はまだ何もしません。
