...
発行されたときに `true` で、ゲームが Lua に見せてくれるのはそこまでです。どの数値がどう聞こえるかを
耳で確かめた人はまだいないので、約束ではなく調整用のつまみとして扱ってください。
...
ここには期待どおりには働かないものが 3 つあります。1 つは未完成で、2 つは頼んだ以上の範囲に
届きます。いずれもエラーにならず静かに終わるので、無音のサウンドに一晩溶かす前に読んで
おいてください。
...
3 つ目は上で触れた `:setVolume` で、`:stop` と同じアクター単位の範囲を持ちます。呼び出した
サウンド 1 つではなく、そのアクターが出しているものすべての音量を変えます。サウンドごとの音量
