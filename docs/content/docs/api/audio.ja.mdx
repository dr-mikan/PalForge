...
黙らせていました。しかもこのツリーが計測したどのビルドでも再生できたことはありません。エンジン側の
両方の経路が閉じています。`USoundWave` はインポーターを宣言しておらず、`USoundWaveProcedural` は
そもそも何も宣言していないので、製品ビルドで `.wav` を読む手段は残っていません。Wwise 側の
`UWwiseExternalSourceStatics::SetExternalSourceMediaBy*` は、Wwise のクックがすでに用意した
メディアを差し替えるものであって、パックのディスク上のファイルを読むものではありません。
