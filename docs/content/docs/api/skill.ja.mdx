...
<Callout type="warn">
この配列はただのリストです。PalForge は id を解決しませんし、パッシブを装備することも、アクティブを
発動することもありません。リストを保持して `pal:skillsOf()` で返すだけです。続きはハンドラで書きます。
下のループだけで足ります。
</Callout>
...
- `pal:skillsOf()` で Pal が持つ id が読めます。そこから回すのは自分の仕事です。
