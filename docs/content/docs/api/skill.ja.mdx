...
装備していないだけかもしれません。テーブルごと `nil` の場合は、そのキャラクターをまったく読めな
かったという意味で、「持っていない」ではありません。添字を引く前に `nil` を確かめてください。
...
<Callout type="info">
尋ねる相手は**パル**にしてください。村人や商人もキャラクターであり、ゲームのキャラクター一覧にも
入っていますが、技を持っていません。そのため尋ねると、読み取りに失敗したときとまったく同じ形の
空のリストが返ってきます。
</Callout>
...
<Callout type="warn">
決めつけずに、その真偽値を確かめてください。手持ちを読むほうは決着していますが、書き込むほうは
まだパックの土台にするような段階ではありません。`:teach` の `false` は、あなたのコードのバグでは
なくフレームワークからの報告です。頼りにする前に、作り直してもよいセーブで一度試してください。
</Callout>
