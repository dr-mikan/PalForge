...
| テクスチャ | **未検証** | `ImportFileAsTexture2D` は出荷バイナリから読んだシグネチャで配線済みですが、このツリーのどの実行も一度も呼んでいません。`/Game/...` のテクスチャなら今日から使えます |
...
<Callout type="warn">
色が実際に変わるところは誰も見ていません。書き込みは宣言どおりに呼ばれているので「書き込みが
走らなかった」は除外できますし、上の名前は推測ではなく実測です。しかしティントが画面に乗ることは
3 つ目の観測であり、それはまだ行われていません。それを記録するのが
`pf_hook mesh-color-change` です。
</Callout>
...
- 塗り用の 4 つのフィールドはすべてのバックエンドに届き、書き込むパラメータ名は動いている
  ゲームから読み取ったものです。色が変わるところはまだ誰も見ていません。
