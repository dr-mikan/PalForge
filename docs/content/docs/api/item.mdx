...
- Hand out a reward once the player has collected enough of a material
...
the same item inside that window is lost. A negative count is a removal, not a pickup, so a
`:take` is skipped rather than reported as an obtain.
</Callout>
...
- The first argument is the handle, not the definition. `:give`, `:take` and the queries are
  right there on it.
...
| `:take(count)` | boolean | Tries to remove `count` (default 1); `count` is treated as a magnitude. `true` only when the inventory count was seen to fall. |
...
`:give` puts items in the player's inventory through the game's own server call. PalForge
resolves the id, finds the local player's inventory, and calls `AddItem_ServerInternal`.
...
Every engine call is wrapped so the helper returns a boolean and never throws; the reason for
a `false` goes to the log. A namespaced id is resolved first, so
`Item.get("example:Potion"):give(1)` hands the name `example_Potion` to the engine — PalForge
does not check that a matching data row exists. A `true` means the native call ran without
raising, which is as far as the game lets Lua see. It is not a report of what the inventory
now holds.

<Callout type="info">
`AddItem_ServerInternal` is also one of the calls that reports a pickup, so a `:give` you make
from ordinary code normally reaches `onObtain` as well. Inside an `onObtain` handler it does
not: a guard stops a handler from setting itself off again.
</Callout>

### take

`:take` is less certain than `:give`. No removal call has ever been confirmed on this build,
so `:take` pushes a NEGATIVE amount through the same `AddItem_ServerInternal`, hoping the game
accepts it. Because that is a hope, the result is measured rather than assumed: the inventory
count is read before and after, and `:take` returns whether it really fell.
...
Every outcome is logged, so the log separates the cases the boolean cannot:
...
<Callout type="warn">
A partial removal still returns `true` — the second line above returns `true` with 2 of 3
gone. `true` means "something was seen to leave", not "exactly `count` left". When the amount
has to be exact, read the count yourself before and after.
</Callout>
...
native enum and no call to apply one has been confirmed, so nothing appears on the status bar
and no HP moves on its own. Whatever the effect is meant to do belongs in `onTick`.
</Callout>
...
Because `take` measures what happened, the `false` branch above really does mean no wood was
seen to leave, and the arrows are not handed out. What a `true` does not promise is that all
5 left — a partial removal returns `true` too — so a trade that has to be exact still needs
its own count check.
...
**Removal is unconfirmed.** No removal call is known on this build. `:take` pushes a negative
amount through the add call and reports what the inventory count did, so `true` means
"something was seen to leave" and never "exactly `count` left".
...
**`give` and `take` work on the local player only.** They find `PalPlayerCharacter` and act
on that inventory. Both return `false` when a step fails and log the reason; `:take` also
returns `false` when every step worked but the count did not fall.

**A `:give` normally fires `onObtain` too.** It goes through the same call the game uses for
a pickup. Inside an `onObtain` handler a guard prevents that, so a handler cannot set itself
off; anywhere else, expect your own give to come back around.
...
- Write `Item{ ... }` to describe an item. Only `id` is required, and the id must be one the
  game already has.
- `Item.get("Wood")` gives you a handle for any item. `:give` adds to the player inventory;
  `:take` tries to remove and tells you whether the count really fell.
