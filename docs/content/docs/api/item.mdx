...
The consume runs through a weapon the player is carrying. A player with nothing equipped at all
has nothing to route it through, so `:take` answers `false` and says exactly that in the log. It
is the one thing to design around: if your building charges a price, a player standing there
bare-handed cannot pay it.
</Callout>
...
texture off the row, so a vanilla id hands you the game's own artwork for that item. Every step
is fail-soft: a missing table, row or column gives you the `icon` you declared instead, which is
`nil` unless you set one. A namespaced id of your own has no row, so it always falls back.
