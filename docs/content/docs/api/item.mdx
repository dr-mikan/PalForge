...
Three items are described up front: `Wood`, `Berries` and `Arrow`. Every other catalog id
becomes a handle the first time you ask `items.get(id)` for it, and the handle is kept for
next time.
...
not make the item craftable in game; it gives your own code somewhere to keep the numbers.
</Callout>

### Validation

Every problem with your table is an error, so a call never half-succeeds. The message starts
with `PalForge: ` and names the field it stopped on.
...
<TypeTable
  type={{
    ...
    onCraft: {
      description: 'You can declare it, but nothing emits item.craft yet, so it never runs.',
      type: 'fun(self: Item.Handle, ctx: table)',
    },
    onDiscard: {
      description: 'You can declare it, but nothing emits item.discard yet, so it never runs.',
      type: 'fun(self: Item.Handle, ctx: table)',
    },
  }}
/>

Three of the game's own calls feed these handlers. Two of them report a pickup, one reports
a use:
...
event and looks up the definition filed under that id. If nothing is filed there, it walks
the items you have described and takes the one whose resolved name matches. A vanilla item
you never described matches nothing, and no handler runs.

### Namespaced ids

An item you describe as `"example:Potion"` is filed under that exact spelling, while the game
reports the data row name `example_Potion`. PalForge closes the gap for you: when the exact
lookup misses, it resolves each id you registered and returns the one that matches. Your pack
...
The search runs over a snapshot of the registered items in no particular order and returns
the first match, so two ids that resolve to the same row name are ambiguous. Keep your pack
ids unique. Pals are matched the same way, by their blueprint class name.
...
On a dedicated server `ctx.actor` is whichever player pawn is found first, which need not be
the one who acted. Treat it as a player pawn to act on, not as proof of who acted.
</Callout>
...
<Callout type="warn">
`onCraft` and `onDiscard` never run. The `item.craft` and `item.discard` channels exist and
your handler is wired to them, but nothing in the game reports either yet: crafting finishes
through a work process rather than the obtained-items log, and no discard call has been
found. You can declare them without an error, so keep the code if you want — but put
anything you need today in `onObtain`, in `onUse`, or in your own code.
</Callout>
...
| `:take(count)` | boolean | Removes `count` (default 1) by dropping it on the ground at the player's feet. `count` is treated as a magnitude. `true` only when the count was seen to fall. |
...
`:give` puts items in the player's inventory through the game's own admin object, and then
checks that they arrived. The inventory count is read before the call and again after it, and
`true` means the count was seen to **rise**.

```mermaid
flowchart LR
  A["Item.Handle:give"] --> B["read the count"]
  B --> C["the game's GetItem cheat"]
...
<Callout type="warn">
`:give` and `:take` go through the game's admin object, which the companion mod
`CheatManagerEnablerMod` switches on. Without it there is no admin object to call, so both are
refused before they reach the game and answer `false`.
[Getting started](/docs/getting-started) has the `mods.txt` line that turns it on.
</Callout>

The boolean is the measurement, never the call. A `false` means nothing was seen to arrive, and
that has several causes — no admin object, an item id the game does not know, or an inventory
with no room for it. Every outcome is logged with the item, the count and what the inventory
did, so the log separates the cases the boolean cannot:

```text
[PalForge.items][info] give Wood x10: 135 -> 145 [evidence declared]
[PalForge.items][warn] give Wood x10: GetItem executed [evidence declared] and the count did not rise (135 -> 135) — unknown item id, or the inventory had no room
[PalForge.items][err] give Wood x10 failed: no PalCheatManager on this session (needs CheatManagerEnabler)
```
...
the game does not know is accepted, adds nothing, and comes back as a `false`.

<Callout type="info">
Do not count on your own `:give` reaching your own `onObtain`. `onObtain` follows the game's own
pickup reporting, and inside an `onObtain` handler a guard stops a handler from setting itself
off again. Log what you hand out at the point you hand it out.
</Callout>

### take

`:take` is the same measurement in the other direction: the count is read before and after, and
`true` means it was seen to **fall**.

<Callout type="warn" title="Taking items drops them at the player's feet">
Nothing in this version makes an item vanish, so `:take` uses the call that takes it out of the
inventory: the items land on the ground where the player is standing. The inventory really has
lost them — that is what `:take` means, and the count check proves it — but a pile appears next
to the player, and anyone can walk over it and pick it back up.

Plan for the pile. `:take` fits "hand this over" well and "this is consumed" badly, so charge a
price with it only where the player picking it straight back up does not break your design.
</Callout>
...
if Item.get("Wood"):take(3) then
    -- three Wood left the inventory, and are lying at the player's feet
else
...
inventory holds. Asking when they hold none skips the call altogether. Both outcomes are logged,
so the log separates the cases the boolean cannot:

```text
[PalForge.items][info] take Wood x3: 12 -> 9, dropped on the ground at the player's feet [evidence declared]
[PalForge.items][warn] take Wood x5: the inventory holds only 2, dropping that many
[PalForge.items][warn] take Wood x3: the inventory holds none, so nothing is dropped
```

<Callout type="warn">
A clamped removal still returns `true` — the second line above drops 2 of the 5 asked for and
...
`count` is the one inventory action that works. It asks the game how many of that id the local
player is carrying and hands back a plain number. `nil` means the count could not be read at
all — no world loaded, no player found — and never that the player has none, so test for `nil`
before you compare.

`iconOf` looks the id up in `/Game/Pal/DataTable/Item/DT_ItemIconDataTable` and reads the
texture off the row. Every step is fail-soft: a missing table, row or column gives you the
`icon` you declared instead, which is `nil` unless you set one.
...
Building{
    ...
    events = {
        onRightClick = function(self, ctx)
            ...
            local ok = Item.get("Wood"):take(5)   -- the 5 Wood land on the ground, not in a bin
            if not ok then
...
Take first and hand over only when the take succeeded: that order is what stops the bench
paying out for free, because `:take` returns `false` when nothing was seen to leave. Two things
it does not promise. The wood does not disappear — it is on the floor next to the player, who
can pick it straight back up, so this bench is a trade of convenience rather than a real price.
And `true` does not mean all 5 left, because a request is clamped to what the player is
carrying, so an exact trade still needs its own `:count()` check.
...
**`onCraft` and `onDiscard` never run.** The channels and the wiring are there, nothing in
the game reports either event.

**A namespaced id is matched by a fallback search.** The event carries the game's data row
name, so an item registered as `"example:Potion"` is not found by the exact lookup and the
search over your registered ids is what finds it. The search reads a snapshot in no
particular order and returns the first id that matches, so two pack ids resolving to the same
row name are ambiguous.

**Taking an item drops it at the player's feet.** `:take` moves items out of the inventory and
into the world, because nothing in this version destroys one. The removal is real and measured,
and so is the pile on the floor.
...
**`category`, `maxStack`, `icon` and `recipe` are PalForge's own notes.** You read them back
through the handle and from your own code. Nothing writes them into the game's item table, so
`maxStack = 9999` does not change how the game stacks that item.
...
**`give`, `take` and `count` work on the local player only.** They find `PalPlayerCharacter`
and act on that inventory, so on a shared server they answer for whichever player pawn is found
first. `count` answering `nil` means the read failed, not that the bag is empty.

**`give` and `take` need the admin object.** Both go through the one
`CheatManagerEnablerMod` switches on. Without that companion mod they are refused before they
reach the game and answer `false`. `count` does not need it.

**Do not count on a `:give` reaching your `onObtain`.** Log what your own code hands out at the
point you call it, rather than waiting for the event to come back around.
...
- Write `Item{ ... }` to describe an item. Only `id` is required, and the id must be one the
  game already has.
- `Item.get("Wood")` gives you a handle for any item. `:count()` reads how many the player is
  carrying, `:give` adds and `:take` removes — and each of the three tells you what it measured,
  not what it attempted. `:take` drops what it removes on the ground.
- Put your code in `events`. `onObtain` runs on pickup, `onUse` runs when the item is used.
  `onCraft` and `onDiscard` do not run yet.
- In `onUse`, `ctx.actor` is the player who used the item — the right target for food and
  potions.
- `category`, `maxStack`, `icon` and `recipe` are notes for your own code. The game does not
  read them.
