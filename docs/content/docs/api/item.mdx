...
`recipeOf()` reads back only what you declared. It does **not** read the game's table, so a
vanilla id answers `nil` even though a real recipe for it exists: `DT_ItemRecipeDataTable_Common`
is loaded under `/Game/Pal/DataTable/Item/` with 1414 rows, keyed by item id, carrying
`Product_Count`, `Material1_Id` through `Material5_Id`, `WorkAmount` and nine more columns. What
is not known is whether a scalar or FName column can be indexed straight off the struct
`FindRow` hands back — the only row value ever read here was a `TSoftObjectPtr`, which cannot be
opened from Lua, and that is what forced the column-as-text detour the icon lookup uses. Reading
the recipe column by column would work today at thirteen calls per table, once, cached. The
question is measured by `pf_hook item-datatable-row-read`, which needs `env.debug` and a loaded
world.
...
| `:recipeOf()` | table \| nil | The recipe you declared, or `nil`. It never reads the game's recipe table. |
...
**`recipeOf` does not read the game's recipe table.** It answers the recipe you declared and
`nil` for a vanilla id, even though the game has a real row for that item.
...
`maxStack = 9999` does not change how the game stacks that item. `icon` is the exception in one
direction only: the live table is read first, and your declared path is the fallback.
...
- In `onUse`, `ctx.actor` is the player who used the item — the right target for food and
  potions.
- `category`, `maxStack` and `recipe` are notes for your own code; the game does not read them,
  and `recipeOf` does not read the game's recipe table either. `icon` is only the fallback for a
  lookup that really does reach the game's icon rows.
