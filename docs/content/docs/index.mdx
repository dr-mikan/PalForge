...
- **Effect** — `onApply`, `onTick`, `onStack` and `onExpire` all run. PalForge keeps the
  clock itself, off its own half-second heartbeat. It does not switch on the game's own
  ailment icons (Poison, Burn, Freeze), so whatever the effect does to the target, your
  handlers do.
...
- `Audio.Handle:setVolume` returns `false` and changes nothing, and a `soundFile` of your
  own resolves but plays nothing. Name a game sound id instead.
