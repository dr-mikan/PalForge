...
<Callout type="error" title="Two things this version cannot do">
Plan around these before you design a pack. Both are calls you can still write — they answer
`false` and change nothing, rather than failing halfway.

- **Pals cannot be placed in the world.** `Pal.Handle:spawn` puts no creature anywhere. What
  does work is reacting to the creatures the game itself spawns — capture, damage, death and
  the periodic sweep all run.
- **A sound has no volume of its own.** `Audio.Handle:setVolume` changes nothing, and the game
  offers no per-sound volume to change. Pick a quieter sound from the catalog instead.

Everything else on this page runs today.
</Callout>
...
- `Item.Handle:give` reports what the inventory was measured to do, so a `false` from it means
  nothing arrived. The `:spawn` line places nothing in this version; everything else here runs.
