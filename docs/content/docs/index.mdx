...
- **Item** — all four run. `onObtain` and `onUse` off the inventory and use paths; `onCraft`
  ...
  hope. `:recipeOf` is the one that still answers nil.
- **Pal** — all five run. `onCaptured`, `onDamaged` and `onDeath` come from confirmed game
  ...
  `PalPlayerCharacter:OnCompleteInitializeParameter`, both observed firing — but every firing so
  far landed in the same second as the world load, so it is unshown that one means a pal that
  did not exist a moment ago. Keep that handler safe to run twice. `onTick` is driven by a sweep
...
- **Mesh** — no lifecycle events. Vanilla `/Game/...` static and skeletal meshes work and are
  class-checked before the engine is handed them, and `.obj` geometry loads from disk. The
  custom-texture import is wired with the right declared signature and has never once been
  called, and nobody has yet watched a colour change happen in game.
- **UI** — no lifecycle events, and no native "the UI changed" signal has been confirmed
  firing, so nothing refreshes an element for you. Building widgets out of Palworld's own UMG
  kit and mounting them into the game's own in-game UI root is confirmed live.
...
define time.** The engine-native half is closed — `USoundWave` declares no importer — and the
Wwise half rebinds a cooked media cookie rather than reading a `.wav` off your disk. The field
stays declared so the error can name it and name the open item, `audio-custom-file-loader`. Only
...
- Skill `onHit` — both of its candidate sources are measured silent from both sides. Call
