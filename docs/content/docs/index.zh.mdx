...
- **Item** —— 四个都会运行。`onObtain` 和 `onUse` 来自背包与使用路径；`onCraft` 来自两个地图对象
  ...
  `:give`、`:take` 和 `:count` 事后都会把背包再读一遍，所以它们返回的是实测，不是期望。现在还会
  返回 nil 的只有 `:recipeOf`。
- **Pal** —— 五个都会运行。`onCaptured`、`onDamaged` 和 `onDeath` 来自已经确认的游戏调用。
  `onSpawned` 挂在 `PalNPC:OnCompletedInitParam` 和
  `PalPlayerCharacter:OnCompleteInitializeParameter` 上，两者都看到过触发 —— 但迄今为止每一次触发
  都落在世界加载的同一秒里，所以“它代表的是一只刚刚才出现的帕鲁”这一点仍未被证明。请把这个处理函数
  写成跑两次也没问题。`onTick` 由一个遍历世界里所有帕鲁的扫描驱动，大约每三秒一次。声明的 `mesh`
...
- **Mesh** —— 没有生命周期事件。原版 `/Game/...` 的静态和骨骼网格能用，交给引擎之前会先做类检查；
  `.obj` 几何体能从磁盘读进来。自定义贴图的导入按正确的声明签名接好了线，但一次都没被调用过，也还
  没有人在游戏里亲眼看到过一次改色。
- **UI** —— 没有生命周期事件，而且没有任何一个原生的“界面变了”信号被确认会触发，所以没有东西会替
  你刷新元素。用 Palworld 自带的 UMG 组件搭出控件、并挂进游戏本体的游戏内 UI 根节点，这条路是实地
  确认过的。
...
**你自己的声音文件没法播放，而且 `Audio.Spec.soundFile` 现在是定义时的硬错误。** 引擎原生的那一半
是关死的 —— `USoundWave` 没有声明任何导入器 —— 而 Wwise 那一半做的是把一个烘焙好的媒体 cookie 重新
绑定，而不是读你磁盘上的 `.wav`。这个字段仍然保留在声明里，好让错误消息能点它的名，并点出未决项
`audio-custom-file-loader`。能播放的只有游戏里本来就有的声音，一共 1957 个 —— 目录见
...
- Building 的 `onLeftClick` 和 `onBreak` —— 这是以否定的方式定下来的，不是还没做。所有可能拥有这类
  ...
  带着 `reason = "missing"`。
- Skill 的 `onHit` —— 它的两个候选来源从两侧测下来都是沉默的。请从你自己的代码里调用
