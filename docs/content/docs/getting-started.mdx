    ...
    └── PalForge/
        ...
        └── Scripts/
            ...
            └── palforge/
                ...
                ├── test/        <- the whole dev test tree (a --release copy has none of it)
                │   ├── units/   <- headless suites, run at startup
                │   ├── cases/   <- the in-game API suite, on F1
                │   ├── hooks/   <- the measurements that need the game running
                │   ├── probes/  <- discovery dumps; they pass and fail nothing
                │   └── tools/   <- dev instruments (the dumper behind ps_catalog)
...
at runtime. `--release` deletes `palforge/test/` from the staged copy on top of that — all 58
files of it — so a player's tree has no test directory at all.

Two entries are easy to leave out of a dev copy, and both have consequences that look like
something else:

- **`palforge/test/`.** `core/registry.lua` requires `palforge.test` and calls its `install()`.
  That one call binds **F1**, runs the headless bundle, gives each discovery probe its key,
  registers every `pf_*` console command including `ps_catalog`, and hands `core/autorun` its
  action table. A dev install without this directory has none of them, and the kernel names the
  whole tree on its "dev tooling NOT loaded" line. A second directory once sat beside this one —
  `palforge/tests/`, *plural*, a single character apart — holding the headless bundle and the
  `ps_catalog` dumper, with production code reaching into both. There is one tree now: the
  bundle is `test/units/` and the dumper is `test/tools/catalog.lua`.
...
[PalForge.hooks][info] 25 game-required hook(s) declared; `pf_hooks` prints each one's gate state and what would open it
[PalForge.unittests][info] tests: 8 passed, 0 failed, 0 skipped (8 total)
[PalForge.keyboard][info] bound F1 [keymap: ...]
[PalForge.test][info] dev console command registered: ps_catalog (DataTable dumper, opt-in)
[PalForge.test][info] console commands: pf_hook  pf_hook_audio_custom_file_loader  ...  pf_uiz  pf_watch
[PalForge.test][info] keyboard: F1                   unknown  tests: all suites                        game: the game's key config has not been read
[PalForge.registry][info] dev tooling loaded: dev keybinds incl. F4 unlock-all-technologies, F9 reload, headless unit bundle (test/units), in-game API suite on F1 + probe keys, ps_catalog console command (test/tools), autorun action table, game-required test hooks (palforge.test.hooks)
[PalForge.registry][info] initialized (dev=true, debug=true, 17 class(es) registered)
[PalForge.main][info] ready
```

`tests: 8 passed ...` is the headless unit bundle — `palforge/test/units/`, two suites and eight
checks of pure Lua, which `test/init.lua`'s `install()` runs at boot before anything touches the
game. The `dev tooling loaded:` line beneath it is that same `install()` reporting each piece back
through the kernel: `core/registry.lua` knows one name, `palforge.test`, and makes one call. A dev
install that dropped the directory gets the whole tree listed by name on a `dev tooling NOT
loaded` line instead — and a `--release` copy does not carry it at all, so `absent` there is the
correct state rather than a broken install. That line is printed in both modes and names every
piece that did not load, and why: a key that was never bound and a key that was bound and never
arrived look identical from outside, and it is what tells them apart.
...
tools/deploy.sh --writes               # dev, plus all nine write opt-ins on. Throwaway save only
tools/deploy.sh --release              # no overlay, and any stale copy is deleted by name
```

It prints how many files it wrote, and the two modes differ by the test tree and the one-file
overlay: **130 in dev, 71 with `--release`**.
...
`registry.initialize()` loads the keybind files under `core/keyboard/` and the F9 reload, then
requires `palforge.test` and calls its `install()` — one name, one call. That call runs the
headless unit bundle, binds F1, gives each discovery probe its own key, registers the
`ps_catalog` DataTable dumper and every other `pf_*` console command, and hands `core/autorun`
its action table. Nine keys in total:

| Key | What it does | What it needs on screen |
| --- | --- | --- |
| **F1** | run the in-game API suite: 19 suites, 612 checks | works anywhere, but 31 checks skip without a loaded save |
...
`env.debug` is the second, narrower switch: it loads `palforge/test/hooks/`, the 25 measurements
...
the quickest smoke test there is: it runs all 19 suites, and the summary says how many checks
...
- PalForge lives at `ue4ss/Mods/PalForge/Scripts/`, with `palforge/` next to `main.lua`. Any other
  layout breaks it. A dev copy needs `palforge/test/` and `palforge/autorun.txt` with the rest:
  without the first there is no F1 and no `pf_*` command, without the second there is no keyless
  way in. `--release` drops `palforge/test/` outright, and that is what a player's copy is.
