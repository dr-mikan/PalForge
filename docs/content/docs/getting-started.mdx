...
Keep it on: putting a pal in the world goes through it, and without it `Pal.Handle:spawn` is
refused before it reaches the game — it writes this line and gives up:
...
And a pal, to see an action rather than a handler. Give the spawn a few seconds: the creature
turns up four to eight seconds after the call, wearing the mesh and carrying the handlers you
declared.
...
Wait a few more seconds first. A pal arrives four to eight seconds after `:spawn` returns, and
the log says when it did — see [Pal](/docs/api/pal). If nothing has turned up after that, look
for this line:
...
It means the call was refused before it reached the game. A world spawn needs
`UPalCheatManager`, so turn on `CheatManagerEnablerMod` in `mods.txt`.
