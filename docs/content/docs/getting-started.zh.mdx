...
保持开启：`Item.Handle:give`、`Item.Handle:take` 和 `Pal.Handle:spawn` 都要走这个对象，没有它时，
它们都会在够到游戏之前被拒绝并返回 `false` —— 生成那条路会写下这一行然后放弃：
...
再来一个帕鲁的例子，看的是一个动作而不是处理函数。`:spawn` 在这个版本里什么都不会放下 —— 下面两行
都返回 `false` —— 所以你能看到的是定义本身：游戏自己的鸡现在带上了你的网格和你的处理函数。
...
这个版本里，不管你传什么，`:spawn` 都不会有生物出现 —— 见 [Pal](/docs/api/pal)，那里也写了帕鲁定义
今天还能为你做什么。下面这条警告是更早一步的另一个问题，无论如何都值得修好：

```text
[PalForge.spawn][warn] spawn.pal: no PalCheatManager (CheatManagerEnabler mod missing this session?)
```

它表示这个调用在够到游戏之前就被拒绝了。`:spawn` 需要 `UPalCheatManager`，所以请在 `mods.txt` 里
打开 `CheatManagerEnablerMod`。
