...
想生成帕鲁的话，就让 `CheatManagerEnablerMod` 保持开启。`Pal.Handle:spawn` 走的是游戏自己的
`UPalCheatManager`，所以没有那个模组时，`core/spawn` 会写下这一行然后放弃，返回 `false`：
...
`:spawn` 走的是 `UPalCheatManager`。在 `mods.txt` 里打开 `CheatManagerEnablerMod`。
