    ...
    └── PalForge/
        ...
        └── Scripts/
            ...
            └── palforge/
                ...
                ├── test/        <- 整棵 dev 测试树（--release 的副本里一个都没有）
                │   ├── units/   <- 无界面套件，启动时就跑
                │   ├── cases/   <- 游戏内 API 测试套件，挂在 F1 上
                │   ├── hooks/   <- 游戏不跑就取不到的测量
                │   ├── probes/  <- 调查用的转储；不判定通过或失败
                │   └── tools/   <- dev 工具（ps_catalog 背后的那个导出器）
                └── utils/       <- log, json, file, items
```

这和 `tools/deploy.sh` 复制的清单是同一份：`Scripts/main.lua` 加上整个 `Scripts/palforge/`，减去
`deprecated/` 和 `tmp/` —— 这两个只作参考，运行时用不到。在这之上，`--release` 还会从暂存的副本里
把 `palforge/test/` 连同它的 58 个文件一起删掉：玩家手上的树里没有任何测试目录。

在 dev 副本里有两项很容易漏掉，而且漏掉之后都会表现成别的毛病：

- **`palforge/test/`。** `core/registry.lua` 会 require `palforge.test` 并调用它的 `install()`。
  就这一次调用绑定了 **F1**、跑了无界面测试包、给每个探针分了一个键、注册了包括 `ps_catalog` 在内
  的全部 `pf_*` 控制台命令，并把动作表交给 `core/autorun`。少了这个目录的 dev 安装一样都没有，内核
  会在 “dev tooling NOT loaded” 那一行里把整棵树点名。以前它旁边还有第二个目录 ——
  `palforge/tests/`，*复数*，只差一个字母 —— 装着无界面测试包和 `ps_catalog` 的导出器，而产品代码
  两边都伸手去够。现在只有一棵树：测试包是 `test/units/`，导出器是 `test/tools/catalog.lua`。
...
`tests: 8 passed ...` 来自无界面的单元测试包 `palforge/test/units/` —— 两个纯 Lua 套件、八项检查，
由 `test/init.lua` 的 `install()` 在启动时、在碰到游戏之前跑完。它下面那行 `dev tooling loaded:`
就是同一个 `install()` 把每一块经内核报告回来的结果：`core/registry.lua` 只知道 `palforge.test`
这一个名字，也只调用一次。要是哪份 dev 安装把这个目录漏掉了，内核会把整棵树点名写进
`dev tooling NOT loaded` 那一行；而 `--release` 的副本本来就不带它，所以那里的 `absent` 是正确的
状态，不是装坏了。这一行两种模式下都会打印，逐项列出没有加载的东西和原因：一个从未被绑定的键，
和一个绑定了却从来收不到按键的键，从外面看是一样的，能区分它们的只有这一行。
...
tools/deploy.sh --writes               # dev，并把九个写入同意项全部打开。只在可以丢掉的存档上用
tools/deploy.sh --release              # 不写覆盖文件，并按名字删掉任何残留的旧副本
```

它会打印自己写了多少个文件，两种模式的差别就是测试树加上那一个覆盖文件：**dev 是 130 个，
`--release` 是 71 个。**
...
`registry.initialize()` 会加载 `core/keyboard/` 下的按键绑定文件和 F9 重新加载，然后 require
`palforge.test` 并调用它的 `install()` —— 一个名字，一次调用。这次调用跑了无界面单元测试包、绑定
F1、给每个探针各分一个键、注册 `ps_catalog` DataTable 导出工具和其余所有 `pf_*` 控制台命令，并把
动作表交给 `core/autorun`。一共九个键：

| 键 | 做什么 | 屏幕上需要什么 |
| --- | --- | --- |
| **F1** | 运行游戏内 API 测试套件：19 个套件，612 项检查 | 哪里都能跑，但没有加载存档时会跳过 31 项 |
...
`env.debug` 是第二个、也更窄的开关：它加载 `palforge/test/hooks/`，也就是那 25 项没有游戏在跑就
...
方向回答同一个问题。做了 dev 部署的话，最快的冒烟测试就是 **F1**：它会跑完 19 个套件，而摘要会写
...
- PalForge 放在 `ue4ss/Mods/PalForge/Scripts/`，`palforge/` 和 `main.lua` 并排。换成别的布局就
  不能用。dev 副本还要一起复制 `palforge/test/` 和 `palforge/autorun.txt`：少了前者就没有 F1、也
  没有任何 `pf_*` 命令，少了后者就没有不靠按键的入口。`--release` 会整个丢掉 `palforge/test/`，
  那才是玩家手上的副本。
