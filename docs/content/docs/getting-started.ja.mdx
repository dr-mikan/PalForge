...
使わせるための Mod です。有効なままにしてください。`Item.Handle:give`、`Item.Handle:take`、
`Pal.Handle:spawn` はいずれもこのオブジェクトを通るので、無いとどれもゲームに届く前に断られて
`false` になります。スポーンの経路は次の行を出して諦めます。
...
パルの例では、ハンドラではなくアクションを見てみます。`:spawn` はこのバージョンでは何も配置しない
ので（下の行はどちらも `false` を返します）、確認できるのは定義そのものです。ゲーム自身のチキンが、
あなたのメッシュとハンドラを持つようになります。
...
このバージョンでは、何を渡しても `:spawn` からクリーチャーは現れません。[Pal](/docs/api/pal) を
参照してください。パルの定義が今も何をしてくれるかもそこに書いてあります。次の警告はそれより手前の
別の問題で、こちらは直す価値があります。
...
これは、呼び出しがゲームに届く前に拒否されたという意味です。`:spawn` には `UPalCheatManager` が
必要なので、`mods.txt` で `CheatManagerEnablerMod` を有効にしてください。
