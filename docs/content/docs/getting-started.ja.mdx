    ...
    └── PalForge/
        ...
        └── Scripts/
            ...
            └── palforge/
                ...
                ├── test/        <- dev 用テストツリー全体（--release のコピーには存在しない）
                │   ├── units/   <- ヘッドレスのスイート。起動時に走る
                │   ├── cases/   <- ゲーム内 API スイート。F1 で走る
                │   ├── hooks/   <- ゲームが動いていないと取れない計測
                │   ├── probes/  <- 調査用ダンプ。合否は一切出さない
                │   └── tools/   <- dev 用の道具（ps_catalog の中身のダンパー）
...
なります。`--release` はさらに、ステージしたコピーから `palforge/test/` を 58 ファイルまるごと
削除します。つまりプレイヤーの手元にテストディレクトリは残りません。

dev のコピーで抜けやすいエントリが 2 つあり、どちらも別の症状に化けます。

- **`palforge/test/`**。`core/registry.lua` は `palforge.test` を require し、その `install()`
  を呼びます。この 1 回の呼び出しが **F1** を割り当て、ヘッドレスバンドルを走らせ、各調査プローブ
  にキーを与え、`ps_catalog` を含むすべての `pf_*` コンソールコマンドを登録し、`core/autorun`
  にアクションテーブルを渡します。このディレクトリが無い dev インストールにはそのどれも無く、
  カーネルは「dev tooling NOT loaded」の行にツリーごと名前を挙げます。以前はこの隣にもう 1 つ、
  1 文字違いの `palforge/tests/`（*複数形*）があり、ヘッドレスバンドルと `ps_catalog` のダンパー
  を持っていて、製品コードが両方に手を伸ばしていました。いまツリーは 1 つで、バンドルは
  `test/units/`、ダンパーは `test/tools/catalog.lua` です。
...
`tests: 8 passed ...` はヘッドレスのユニットバンドル `palforge/test/units/` です。純 Lua の
スイート 2 本・チェック 8 件で、`test/init.lua` の `install()` が起動時、ゲームに触れる前に
走らせます。
その下の `dev tooling loaded:` の行は、同じ `install()` が各パーツをカーネル経由で報告したもの
です。`core/registry.lua` が知っている名前は `palforge.test` の 1 つだけで、呼び出しも 1 回だけ
です。このディレクトリを落とした dev インストールでは、代わりにカーネルが
`dev tooling NOT loaded` の行にツリーごと名前で挙げます。`--release` のコピーはそもそも持って
いないので、そこでの `absent` は壊れたインストールではなく正しい状態です。この行は両方のモードで
出力され、ロードされなかったものとその理由をすべて名前で列挙します。バインドされなかったキーと、
バインドされたのに届かなかったキーは外から見ると同じに見えるので、両者を区別できるのはこの行だけ
です。
...
tools/deploy.sh --writes               # dev に加えて書き込み系オプトイン 9 つを全部 on。捨ててよいセーブ専用
tools/deploy.sh --release              # オーバーレイ無し。古いコピーは名前を挙げて削除する
```

書き出したファイル数を出力します。2 つのモードの差はテストツリーと 1 ファイルのオーバーレイで、
**dev では 130、`--release` では 71** です。
...
`registry.initialize()` は `core/keyboard/` 以下のキーバインドファイルと F9 のリロードを読み込み、
そのあと `palforge.test` を require して `install()` を呼びます。名前 1 つ、呼び出し 1 回です。
その呼び出しがヘッドレスのユニットバンドルを走らせ、F1 を割り当て、各 discovery プローブにキーを
1 つずつ与え、`ps_catalog` DataTable ダンパーとその他すべての `pf_*` コンソールコマンドを登録し、
`core/autorun` にアクションテーブルを渡します。キーは合計 9 個です。

| キー | 何をするか | 画面に何が必要か |
| --- | --- | --- |
| **F1** | ゲーム内 API スイートの実行。19 スイート、612 チェック | どこでも動くが、セーブをロードしていないと 31 チェックが skip |
...
`env.debug` は 2 つ目の、より狭いスイッチです。ゲームが動いていないと計測できない 25 件の測定を
...
です。19 スイートすべてを実行し、答えを出せなかったチェックが何件で、どちら方向なのかをサマリが
書きます。
...
- PalForge は `ue4ss/Mods/PalForge/Scripts/` に置き、`palforge/` を `main.lua` の隣に置きます。
  配置が違うと動きません。dev のコピーには `palforge/test/` と `palforge/autorun.txt` も一緒に
  必要です。前者が無いと F1 も `pf_*` コマンドも存在せず、後者が無いとキー無しで入る手段が
  無くなります。`--release` は `palforge/test/` を丸ごと落とし、それがプレイヤーの手元のコピーです。
