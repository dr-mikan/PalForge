...
パルをスポーンさせたい場合は `CheatManagerEnablerMod` を有効なままにしてください。`Pal.Handle:spawn`
はゲーム本体の `UPalCheatManager` を経由するため、この Mod がないと `core/spawn` は次の行を出して
`false` を返し、何もしません。
...
- ゲーム内テストスイート `palforge.test` を読み込み、**F1** に割り当てます。キーを押すまで何も実行され
  ません。これらのテストはパルをスポーンさせたりアイテムを配ったりするからです。
