...
--   3. READING A NAME DOES NOT REGISTER A CATALOG. The whole design rests on laziness: 8261
--      eager definitions at mod load would fill object_manager with content nobody asked for.
--      This measures the registry across a read instead of trusting the comment.
--
...
--=============================================================================
-- laziness: the whole reason the fields are a metatable and not 8261 definitions
--=============================================================================

s:test("reading one name registers one class, not a catalog", function(t)
    ...
    t:eq(om.get("building", ROW), nil, "the row must start unregistered — the catalog is data")
    ...
    t:eq(h.id, ROW)
    t:truthy(om.get("building", ROW), "reading the name must have defined exactly that row")
    t:eq(after - before, 1, "and exactly one class more than before, not 498")

    -- reading it again defines nothing further: get(id) is the cache the metatable rides on
    local again = buildings[ROW]
    t:eq(again, h, "the second read must be the same handle")
    t:eq(registeredCount(), after, "and must not register anything at all")
end)

s:test("the catalogs are still overwhelmingly unregistered after this suite has run", function(t)
    -- The kernel loads six catalogs holding 8261 rows between them. If loading one ever started
